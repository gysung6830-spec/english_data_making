"""simp_g*.yaml(챕터→[{id,how}])의 간결화된 how만 예문 소스에 되쓴다(cut 유지)."""
...
merged = {}
for f in sorted(SP.glob("simp_g*.yaml")):
    d = yaml.safe_load(f.read_text(encoding="utf-8")) or {}
    for k, v in d.items():
        merged.setdefault(k, {})
        for row in v or []:
            merged[k][row["id"]] = row.get("how", "")
...
print(f"how 간결화 적용: 1부 {a} · 3부 {b} · 어휘유추 {c} = {a+b+c}개")
