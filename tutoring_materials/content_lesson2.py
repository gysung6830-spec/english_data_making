...
# ── 심화 참/거짓 — 기존 tf 덮어쓰기 ──
_HARD_TF = {
 11: [("전 세계 인구의 약 15%가 어떤 형태의 장애를 겪는다.", True),
      ("두 청소년은 국내 과학 대회에서 발명품을 선보였다.", False),
      ("Seoyoung은 두 눈을 모두 떠야만 공간에서 위치를 인식할 수 있음을 깨달았다.", False),
      ("많은 장애인에게 접근성은 여전히 해결되지 않은 문제다.", True)],
 12: [("시각을 잃은 사람들은 주로 촉각을 통해 공간 정보를 얻는다.", True),
      ("뇌는 시각이 있어야만 3차원 정보를 처리할 수 있다.", False),
      ("이 장치는 초음파를 쏘아 주변 정보를 모은다.", False),
      ("반사된 빛이 정보를 전달하면 장치가 핀을 올린다.", True)],
 13: [("장치의 핵심은 작은 컴퓨터에 연결된 3차원 심도 감지기다.", True),
      ("감지기는 앞 공간을 약 여덟 칸으로 나눠 훑는다.", False),
      ("장애물의 높이에 따라 해당 핀이 중간 또는 위 위치로 올라온다.", True),
      ("사용자는 핀을 눈으로 보고 장애물의 위치를 파악한다.", False)],
 14: [("장치가 처음 작동했을 때, Seoyoung은 실수인지 확인하려고 껐다 다시 켰다.", True),
      ("연구·제작·시험에는 약 7주가 걸렸다.", False),
      ("그녀는 핀이 더 적은 단순한 미래 버전을 구상하고 있다.", False),
      ("그녀가 상상하는 미래 버전은 스마트폰에 연결될 수 있다.", True)],
 15: [("Nand는 기계 장치가 꼭 필요한 발명을 원했다.", False),
      ("친구들은 끝까지 함께 수어 앱을 개발해 완성했다.", False),
      ("그는 몇 달 만에 실시간 ASL 번역 앱을 개발했다.", True),
      ("가장 어려운 부분은 앱이 손동작을 인식하게 만드는 방법을 찾는 것이었다.", True)],
 16: [("Nand는 막힐 때 잠을 자며 생각했고, 그 방법이 도움이 되었다.", True),
      ("그는 손가락 끝처럼 수어에 중요한 신체 부위에 초점을 맞췄다.", True),
      ("카메라가 포착한 동작은 문자로 바뀌지 않고 그대로 저장된다.", False),
      ("그는 기계학습으로 앱이 영상 속 주요 지점을 식별하도록 학습시켰다.", True)],
 17: [("이 시스템의 수어 번역 정확도는 약 90.4%이다.", True),
      ("수어에서 번역까지 걸리는 시간은 약 3초다.", False),
      ("두 발명품의 공통점은 기술을 창의적으로 활용했다는 점이다.", True),
      ("Nand는 이 기술이 기존 의사소통 플랫폼과는 무관하기를 바란다.", False)],
 18: [("자율주행차의 목표는 인간처럼 생각하고 운전하는 것이다.", True),
      ("자율주행차 기술은 점점 더 접근하기 어려워지고 있다.", False),
      ("자율주행차가 있으면 나이나 능력에 따른 제한이 줄어든다.", True),
      ("시각장애인은 자율주행차를 이용해 이동할 수 없다.", False)],
 19: [("대부분의 사고는 도로 위의 부주의한 사람들 때문에 발생한다.", True),
      ("자율주행 기술은 이미 완벽한 수준에 도달했다.", False),
      ("자율주행차는 센서·GPS·컴퓨터 프로그램에 의존해 작동한다.", True),
      ("센서 등이 고장 나도 탑승자의 안전에는 아무 문제가 없다.", False)],
}
for _d in DAYS:
    if _d["day"] in _HARD_TF:
        _d["tf"] = _HARD_TF[_d["day"]]
