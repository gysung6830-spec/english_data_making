...
# ── 심화 참/거짓 (숫자·개념 함정·부정·추론형) — 기존 tf 덮어쓰기 ──
_HARD_TF = {
 1: [("모기는 날개를 1초에 약 600번 퍼덕이며 다가온다.", True),
     ("물린 뒤 남는 붉은 자국은 모기의 침에 대한 ‘심각한’ 알레르기 반응이다.", False),
     ("지문에 따르면, 긁을수록 가려움은 오히려 더 심해진다.", True),
     ("모기는 사람이 들이마시는 산소 냄새를 맡고 찾아온다.", False)],
 2: [("모기는 이산화탄소를 가까이서만 감지할 수 있다.", False),
     ("올라간 체온도 모기가 알아채는 단서 중 하나다.", True),
     ("땀 속 화학물질은 모기를 쫓아내는 역할을 한다.", False),
     ("수컷과 암컷 모두 알을 낳기 위해 피를 빤다.", False)],
 3: [("암컷 모기는 피를 빤 뒤 주로 수평인 바닥에 내려앉는다.", False),
     ("모기는 중력을 이용해 빨아들인 피에서 수분을 빼낸다.", True),
     ("모기는 자기 체중의 약 두 배에 달하는 피를 빤다.", False),
     ("모기는 마른 땅이 아니라 물웅덩이 표면에 알을 낳는다.", True)],
 4: [("Winegard에 따르면 모기가 죽인 사람은 지금까지 산 인류의 약 절반에 해당한다.", True),
     ("매년 전 세계에서 약 백 명이 이 질병들로 죽는다.", False),
     ("서로마 제국의 몰락은 갑작스럽게 한꺼번에 일어났다.", False),
     ("모기는 질병만 옮겼을 뿐 역사에는 영향을 주지 않았다.", False)],
 5: [("습지는 모기에게는 이상적이지만 말라리아 확산에는 불리한 곳이었다.", False),
     ("모기는 처음엔 로마를 공격하는 군대를 막는 데 도움이 되기도 했다.", True),
     ("모기는 질병을 도시 안에만 퍼뜨리고 제국 전체로는 번지지 않았다.", False),
     ("말라리아 같은 질병도 로마의 몰락에 기여했다.", True)],
 6: [("스코틀랜드 배는 정착민 약 1,200명과 값진 물자를 싣고 떠났다.", True),
     ("정착민들은 현지 질병에 어느 정도 면역이 있었다.", False),
     ("다리엔 계획은 식량난 속에서 경제를 살리려는 시도였다.", True),
     ("6개월 뒤 생존자들은 그 지역에 정착지를 완성했다.", False)],
 7: [("인류는 20세기에 들어서야 모기가 말라리아를 옮긴다는 걸 알아냈다.", False),
     ("면역계가 더 강했더라면 그렇게 많이 죽지 않았을 것이라고 말한다.", True),
     ("사람들은 수천 년간 병의 전파 방식을 알면서도 대처하지 못했다.", False),
     ("글쓴이는 인간의 역사가 자연의 작용과 무관하다고 본다.", False)],
 8: [("무당벌레 한 마리는 평생 진딧물을 약 오백 마리 먹는다.", False),
     ("무당벌레는 정원의 이로운 벌레 중 가장 잘 알려져 있다.", True),
     ("고수나 박하를 심으면 무당벌레를 쫓아낼 수 있다.", False),
     ("무당벌레는 천적이 싫어하는 냄새를 뿜어 스스로를 지킨다.", True)],
 9: [("지렁이는 굴을 파서 흙에 공기가 통하게 하고 뿌리에 도움을 준다.", True),
     ("지렁이의 배설물은 흙을 더 척박하게(메마르게) 만든다.", False),
     ("지렁이를 들인 뒤에는 인공 비료를 마음껏 써도 된다.", False),
     ("주변 흙이 크게 바뀌어도 지렁이는 그 자리에 계속 머문다.", False)],
}
for _d in DAYS:
