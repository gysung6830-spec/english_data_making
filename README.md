...
영어 지문 PDF를 넣으면 **분석지(6개 섹션)·어휘 리스트·영단어 시험지·모의고사 훈련서** 4종 PDF를 자동으로 만들어 주는 개인용 도구입니다.
...
| `outputs.train` | 모의고사 훈련서(입문·60점대) 생성 여부 — 지문당 API 1회 추가 |
...
4. **모의고사 훈련서** (`(지문명)_모의고사훈련서.pdf`) — 아래 참고

### 🏋️ 모의고사 훈련서 (입문·60점대 학생용)

모의고사를 처음 푸는 학생, 60점대 학생을 위한 **기초 훈련 워크북**입니다.
"소재를 찾고 → 주제를 잡고 → 선지(보기)를 읽어 걸러내는" 실전 감각을 단계별로 길러 줍니다.

- **STEP 1 소재 찾기** — 반복·강조 어구를 단서로 '무엇에 관한 글인지' 잡는 연습
- **STEP 2 주제 찾기** — 소재 → 주제로 가는 생각의 순서(안내) + 직접 한 문장 쓰기
- **STEP 3 유형 문제** — 주제·요지·빈칸 5지선다. 선지마다 왜 정답/오답인지(선지 읽기 훈련)
- **STEP 4 구문 훈련** — 끊어 읽기 + 구문 힌트를 보고 직접 직독직해 해 보기
- **정답·해설** — 소재/주제 정답, 선지별 O/X 분석, 풀이 과정, 구문 훈련 해석(맨 뒤 페이지)

> 훈련서는 지문마다 API 를 한 번 더 호출합니다. 필요 없으면 `config.yaml` 의 `outputs.train: false`
> (또는 웹앱 체크박스 해제)로 끄면 비용이 절약됩니다.
