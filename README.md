...
| **③ 문장별 끊어읽기 & 내용 파악(여백 넉넉·필요시 2페이지)** | 문장을 `/`로 끊고 **통째+단어 이중 빈칸**: 오역 위험이 큰 조각은 **통째로 빈칸(긴 빈칸, 문장당 최대 2개)**, 나머지 조각은 **핵심 단어 1개씩만 단어 단위 빈칸(짧은 빈칸)**. **영어엔 표시하지 않아** 어디가 빠졌는지 찾는 것 자체가 학습 · **어법 칩**(학생용은 해설을 숨기고 **‘문장에서 찾아 ○표’ 지시**, 강사용은 해설 표시) · **‘이렇게 읽으면 오답’ → 문장 (X)**(이것만 풀어도 이해되도록: **모든 문장에 핵심의미+필자의견**, 지문 전체에 **지칭 오인·함축의미 각 최소 1개** 보장) | ○(빈칸·오답찾기) | ○(정답·이유) |
| **④ 글 정리(한 페이지)** | 🔎소재 · 🗣**필자 의견** · 🧩**글의 구조**(각각 별도 카드) · 🔗**재진술 사슬(원문 지문에 형광펜 + 소재 번호 — 흑백 인쇄에서도 구분)** — 학생용은 **각 소재의 첫 표현만 번호·형광펜으로 예시** 표시하고 나머지는 직접 · 📝**글 내용 정리(글 순서)** — 도입→전개→사례… 블록 요약·쉬운 예시(학생용은 요약 핵심어 빈칸) | ○(찾기·빈칸) | ○(정답) |
| **⑤ 이 지문의 핵심 문법** | 핵심 문법 1개 미니 강의(별도 페이지): 맨 위에 **이 문법이 나온 지문 문장 박스**(+문장 번호·분석) · **신호어 칩('쉽게 말하면'·'왜 그래?'·'생략'·'주의' 등) + 친근한 설명 3~5줄**(**학생용은 핵심어 빈칸**) + **연습문제 5개(객관식 3 · 영작 2, 1단 배치·한 페이지)** — 객관식은 **지문 1 + 응용 2**, 영작은 **지문 1 + 응용 1**(제시어 제공). 객관식 정답은 보기에 ✓ 표시. 핵심 문법은 **우선순위(가정법→도치→비교→강조→분사구문→병렬→관계사→접속사→부정사·동명사→수동태)**로 선정 | ○(빈칸·풀이) | ○(정답) |

- 학생용은 정답이 될 수 있는 **상단 제목(소재)을 감추고**, 강사용은 표시합니다.
- ③·④·⑤는 각각 **새 페이지**에서 시작합니다. **④·⑤는 한 페이지**, **③은 여백을 넉넉히 두어 필요하면 2페이지**로 흐릅니다(⑤ 연습문제는 1단 배치). 폰트는 **나눔스퀘어라운드**(문서에 임베드).
