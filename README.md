...
> 📄 이 저장소에는 **두 개의 도구**가 있습니다.
> - **지문 분석 도구** (이 문서) — 지문을 넣으면 6가지 *분석 자료* PDF 생성
> - **시험지 자동 생성 툴** → [`시험지생성_README.md`](시험지생성_README.md) — 지문을 넣으면 *수능형 변형문제 7종(순서·삽입·주제·어휘·어법·내용일치·서술형)* 이 담긴 고등 모의고사 스타일 2단 PDF 생성 (`python 시험지생성_데모코드.py` 로 미리보기)
