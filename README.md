...
| **⑤ 이 지문의 핵심 문법** | 핵심 문법 1개 미니 강의(별도 페이지): **친근한 말투의 항목별 설명 3~5줄**(형태·이유·생략·주의·해석 요령 등, **학생용은 핵심어 빈칸**) + 지문 예 & 분석 + **연습문제 9개(객관식 3 · 해석 3 · 영작 3, 2단 배치)** — 객관식은 **지문 문장을 직접 인용**해 출제, 영작은 **지문 문장을 복원**(제시어 제공). 핵심 문법은 **우선순위(관계사절→도치·생략→분사구문→…)**로 선정 | ○(빈칸·풀이) | ○(정답) |
