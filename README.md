...
| **③ 문장별 끊어읽기 & 내용 파악(한 페이지)** | 문장을 `/`로 끊고 **통째+단어 이중 빈칸**: 오역 위험이 큰 조각은 **통째로 빈칸(긴 빈칸, 문장당 최대 2개)**, 나머지 조각은 **핵심 단어 1개씩만 단어 단위 빈칸(짧은 빈칸)**. **영어엔 표시하지 않아** 어디가 빠졌는지 찾는 것 자체가 학습 · **어법 칩** · **‘이렇게 읽으면 오답’ → 문장 (X)**(이것만 풀어도 이해되도록: **모든 문장에 핵심의미+필자의견**, 지문 전체에 **지칭 오인·함축의미 각 최소 1개** 보장) | ○(빈칸·오답찾기) | ○(정답·이유) |
...
- ③·④·⑤는 각각 **새 페이지**에서 시작하며 **한 페이지로 압축**됩니다(⑤ 연습문제는 2단 배치). 폰트는 **나눔스퀘어라운드**(문서에 임베드).
