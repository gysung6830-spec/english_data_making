...
| 파트 | 내용 | 학생용 | 강사용 |
|---|---|:---:|:---:|
| **🔑 핵심 문법(지문 앞)** | 이 지문에서 가장 어려운 핵심 문법 1개 설명 + 지문 속 예 | ○ | ○ |
| **① 지문 통째로 읽기** | 문장 번호 칩이 달린 전체 지문 | ○ | ○ |
| **② 어휘 리스트** | 지문 핵심 어휘를 `S# 단어 뜻`(나온 문장 번호 포함) 압축 리스트로 | ○ | ○ |
| **③ 문장별 끊어읽기 & 내용 파악** | 문장을 `/`로 끊고, **해석 어려운(오역 위험) 부분을 빈칸**(단어~구~조각 통째까지 자유, 다의어·구문 오인·수동/도치/생략·지칭·관용·부정·연결어 등) · **어법 칩** · **‘이렇게 읽으면 오답(X) → 왜?’**(이것만 풀어도 **필자 의견·함축의미·지칭**을 알게 되도록 구성) | ○(빈칸·오답찾기) | ○(정답·이유) |
| **④ 글 정리** | 🔎소재 · 🗣**필자 의견 & 글의 구조(한 카드)** · 🔗**재진술 사슬(원문 지문에 형광펜)** · 📝**글 내용 정리(글 순서)** — 도입→전개→사례… 블록 요약·쉬운 예시(학생용은 요약 핵심어 빈칸) | ○(찾기·빈칸) | ○(정답) |
