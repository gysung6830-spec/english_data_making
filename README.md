...
4. **내신 서술형 대비 교재** (`(지문명)_서술형교재.pdf`) — 아래 6개 유형을 한 PDF에 **학생용 → 교사용 → 빠른 정답 → 정답 및 해설** 4파트로 구성

### 📝 내신 서술형 대비 교재 — 6개 유형

지문 하나에서 실제 학교 시험형 서술형·변형 문제를 만들어, 문제·정답·해설을 한 번에 제공합니다.

1. **서술형 요약문 완성** — 지문 요약문의 빈칸을 원문에서 찾은 단어로 채우기 (어휘·어법·철자 확인)
2. **문장 변형 대비** — 지문 문장을 5개 선지로 paraphrasing 한 뒤 의미가 일치하는 것 고르기 (동의어·반의어·문맥)
3. **요지·제목 배열 영작** — 보기 단어를 어순에 맞게 배열해 요지와 제목 영작
4. **조건 영작 연습** — 주어진 어법 조건과 단어 수에 맞춰 영작
5. **보기 어휘 빈칸 완성** — 지문형 빈칸 (A)~(D)를 `<보기>` 5개로 채울 때 **사용되지 않는** 낱말 고르기 (어휘력·문맥)
6. **어법 오류 수정** — 문장 속 어법상 틀린 부분을 찾아 바르게 고치기
