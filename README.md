...
| **③ 문장별 끊어읽기 & 내용 파악(한 페이지)** | 문장을 `/`로 끊되 **오역 위험이 큰 조각은 통째로 빈칸**, 나머지 조각은 **핵심 단어 1개씩만 단어 단위 빈칸**. **영어엔 표시하지 않아** 어디가 빠졌는지 찾는 것 자체가 학습 · **어법 칩** · **‘이렇게 읽으면 오답’ → 문장 (X)**(이것만 풀어도 **필자 의견·함축의미·지칭**을 알게 되도록 구성) | ○(빈칸·오답찾기) | ○(정답·이유) |
