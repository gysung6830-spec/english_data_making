...
| **④ 글 정리(한 페이지)** | 🔎소재 · 🗣**필자 의견** · 🧩**글의 구조**(각각 별도 카드) · 🔗**재진술 사슬(원문 지문에 형광펜, 소재별 번호)** — 학생용은 **첫 소재만 예시로 칠해 주고** 나머지는 직접 · 📝**글 내용 정리(글 순서)** — 도입→전개→사례… 블록 요약·쉬운 예시(학생용은 요약 핵심어 빈칸) | ○(찾기·빈칸) | ○(정답) |
| **⑤ 이 지문의 핵심 문법** | 핵심 문법 1개 미니 강의(별도 페이지): 맨 위에 **이 문법이 나온 지문 문장 박스**(+문장 번호·분석) · **신호어 칩('쉽게 말하면'·'왜 그래?'·'생략'·'주의' 등) + 친근한 설명 3~5줄**(**학생용은 핵심어 빈칸**) + **연습문제 5개(객관식 3 · 영작 2, 2단 배치)** — 객관식은 **지문 문장을 직접 인용**, 영작은 **지문 문장 1개 + 지문에 없는 응용 1개**(제시어 제공). 핵심 문법은 **우선순위(가정법→도치→비교→강조→분사구문→병렬→관계사→접속사→부정사·동명사→수동태)**로 선정 | ○(빈칸·풀이) | ○(정답) |
