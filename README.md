...
강사가 문장을 하나하나 직독직해로 풀어 주는 대신, **학생이 먼저 스스로 시도하고 확인하는**
훈련 자료를 지문만 넣으면 자동으로 만들어 줍니다. (직독직해 전체 번역은 넣지 않습니다 — 해설지에 이미 있으니까요.)

지문 1개당 **학생용 문제지**와 **강사용 정답지**가 세트로 나오며, 각 지문은 5개 훈련 섹션으로 구성됩니다.

| 섹션 | 학생용(문제) | 강사용(정답) |
|---|---|---|
| **A. 어휘 힌트** | 핵심 단어 4~6개를 `단어 – 뜻`으로만 제시(전체 해석 X) | 동일 |
| **B. 오역포인트 교정 연습** | 오역 위험 문장 2~3개 → 먼저 직접 해석 → 2지선다(흔한 오역/올바른 해석) | 정답 + 오역 유형 태그 + 원인 해설 |
| **C. 문장 역할 파악** | 문장 묶음의 역할을 괄호에 골라 쓰기(통념/전환/근거/예시/결론) | 정답 라벨 + 근거 1줄 |
| **D. 함정포인트 문항** | 지문에 실제 있는 함정 1개(지칭/연결어 반전/생략 주어) 2지선다 | 정답 + 해설 |
| **E. 패러프레이징 줄잇기** | 변형된 선지 ↔ 본문 문장 번호(S1…) 줄로 잇기 | 선지–문장 정답 매핑표 |

- 웹앱에서 **"🎓 강의컨셉 교재"** 를 체크하거나, `config.yaml` 의 `outputs.lecture: true` 로 켭니다.
- 저장 이름: `(지문명)_강의교재_학생용.pdf` · `(지문명)_강의교재_강사용.pdf`
- 문장은 코드가 규칙 기반으로 분리·번호(S1, S2…)를 매기고(약어 `Mr.`·소수점 예외 처리), LLM 은 그 번호만 참조해 문항을 만듭니다. **지문에 실제로 존재하는 요소만** 출제하며(없는 함정·역할은 억지로 만들지 않음), 문장 번호 참조가 지문 범위를 벗어나면 자동으로 재요청합니다.
