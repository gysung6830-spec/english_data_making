...
| `outputs.worksheet` | 단어 학습지(같은 단어 3가지 방법 암기) 생성 여부 |
...
4. **단어 학습지** (`(지문명)_단어학습지.pdf`) — 핵심 어휘를 10개씩 묶어 **같은 단어를 여러 방법으로** 익히는 암기용 학습지
   - **유형①** 소리 내어 읽으며 영어와 뜻을 함께 두 번 쓰기
   - **유형②** 줄 잇기(영어–뜻 연결)
   - **유형③** 뜻만 보고 영어 쓰기(첫 글자 힌트)
   - **유형④** 영어 보고 뜻 쓰기(양방향 암기)
   - **유형⑤** 철자 뒤섞기(Anagram) — 흩어진 글자로 스펠링 완성
   - **유형⑥** 문맥 빈칸 채우기 — 그 단어가 실제로 나온 **지문 문장**에서 단어를 채워 넣기(뜻 힌트 + 세트별 정답 포함)
