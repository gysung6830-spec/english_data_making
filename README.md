...
만든 자료를 파는 **고객용 판매 사이트**가 함께 들어 있습니다.
위의 분석 도구(`webapp.py`)와 분리된 별도 앱이라, 인터넷에 올려도 API 키가 노출되지 않습니다.

```bash
pip install -r store_requirements.txt
python store.py        # http://localhost:5001
```

- 분류는 **모의고사 · EBS 부교재 · 형광펜 독해(자체 교재)** 세 가지 (교과서는 준비 중)
- 자료 목록 · **교재별 페이지** · 상품 상세 · 주문서 · 무료 샘플 · 맞춤 제작 문의
- 화면 전체 **나눔스퀘어라운드** (저장소의 폰트 파일을 직접 제공, CDN 의존 없음)
- **공지사항 · 자료 업데이트 일정** 페이지 (홈 상단에 최신 공지 띠 표시)
- **프리패스**(무제한 이용권) 가격표 + 사전 신청 — 지금은 결제 없이 신청만 받는 모드
- 주문은 사이트 안에서 접수되고 `/admin` 에서 입금 확인 → 발송 처리
- 결제는 계좌이체 안내 방식(주문 시 계좌 표시). 카드 결제는 나중에 붙일 수 있는 구조

상품 등록, 계좌·사업자 정보 입력, 배포 방법은 **[STORE.md](STORE.md)** 에 정리해 두었습니다.
