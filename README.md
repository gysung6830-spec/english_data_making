...
| **③ 문장별 끊어읽기 & 내용 파악(한 페이지)** | 문장을 `/`로 끊되 **오역 위험 조각은 통째로 빈칸**, 나머지는 **단어 위주로 잘게** 끊음. **영어엔 표시하지 않아** 어디가 빠졌는지 찾는 것 자체가 학습 · **어법 칩** · **‘이렇게 읽으면 오답’ → 문장 (X)**(이것만 풀어도 **필자 의견·함축의미·지칭**을 알게 되도록 구성) | ○(빈칸·오답찾기) | ○(정답·이유) |
| **④ 글 정리(한 페이지)** | 🔎소재 · 🗣**필자 의견 & 글의 구조(한 카드)** · 🔗**재진술 사슬(원문 지문에 형광펜)** · 📝**글 내용 정리(글 순서)** — 도입→전개→사례… 블록 요약·쉬운 예시(학생용은 요약 핵심어 빈칸) | ○(찾기·빈칸) | ○(정답) |
| **⑤ 이 지문의 핵심 문법** | 핵심 문법 1개 미니 강의(별도 페이지): **친근한 말투의 항목별 설명 3~5줄**(형태·이유·생략·주의·해석 요령 등, **학생용은 핵심어 빈칸**) + 지문 예 & 분석 + **연습문제 9개(객관식 3 · 해석 3 · 영작 3, 영작은 제시어 제공)** | ○(빈칸·풀이) | ○(정답) |
