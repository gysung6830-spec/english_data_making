...
지문 1개당 **학생용 문제지**(①②③)와 **강사용(①~⑤, 답지 포함)** 이 세트로 나옵니다.
각 지문은 5개 파트로 구성됩니다.

| 파트 | 내용 | 학생용 | 강사용 |
|---|---|:---:|:---:|
| **① 지문 통째로 읽기** | 문장 번호가 매겨진 전체 지문 | ○ | ○ |
| **② 해석 전 예측** | 🔎소재 · 🗗필자 주장(긍/부/중) · 🧩글의 구조(6택) · 🔗재진술 사슬 찾기 → **학생이 빈칸에 예측** | ○(빈칸) | ○(빈칸) |
| **③ 어휘 & 오역 포인트 잡기** | 📘어휘 힌트(주어짐) + **오역 위험 지점만 골라 객관식/짧은 주관식 문제** + ✍한 줄 요약(마무리) | ○(문제) | ○ |
| **④ 답지 — 끊어읽기·캐치** | 💡지문 비유 · 문장 끊어읽기(/)+직독직해 · ✅캐치(필자 의도) · 💡쉬운 예시 | ✕ | ○ |
| **⑤ 정답 & 해설** | 소재/주장/구조/재진술 정답 + 오역 문제 정답·해설 + ✅전체 요지 | ✕ | ○ |

> **③는 지문 전체 해석을 일일이 적게 하지 않습니다.** 틀리기 쉬운 핵심(다의어·구조 오인·비교
> 대상·지칭·수동/도치/생략 등)만 골라 **객관식 또는 짧은 주관식**으로 내므로, 학생은 그것만 풀면
> 그 지문 학습이 마무리됩니다. 객관식 오답은 '학생이 흔히 하는 오역'으로 구성됩니다.

- 웹앱에서 **"🎓 강의컨셉 교재"** 를 체크하거나, `config.yaml` 의 `outputs.lecture: true` 로 켭니다.
- 저장 이름: `(지문명)_강의교재_학생용.pdf` · `(지문명)_강의교재_강사용.pdf`
- 문장은 코드가 규칙 기반으로 분리·번호를 매기고(약어 `Mr.`·소수점·인용부호 예외 처리),
  LLM 이 2단계로 분석합니다: **①개관(소재·주장·구조·재진술·비유·요지) → ②문장별 분석**(개관에서
  정한 비유를 모든 문장의 '쉬운 예시'가 공유). **지문에 실제로 있는 내용만** 근거로 하며,
  문장 분석 개수가 지문 문장 수와 다르면 자동으로 재요청합니다.
