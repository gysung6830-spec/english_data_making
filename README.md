...
만든 자료를 파는 **고객용 판매 사이트 + 관리자 화면**이 함께 들어 있습니다.
위의 분석 도구(`webapp.py`)와 분리된 별도 앱이라, 인터넷에 올려도 API 키가 노출되지 않습니다.

```bash
pip install -r store_requirements.txt
ADMIN_PASSWORD=원하는비번 python store.py
#  고객 화면  http://localhost:5001
#  관리자     http://localhost:5001/admin
```

**고객 화면**
- **오르티카잉 라인업** — 우리가 만드는 자료 8종을 묶음별로 소개
  (지문 이해: 지문자료·지문분석지 / 시그니처: 필생보·필생보 독학용 /
  시험 대비: 통합 워크북·서술형 대비·17종 변형문제·동형모의고사).
  상품마다 어떤 자료가 들어가는지 배지로 표시되고 이 설명으로 이어집니다
- 분류(모의고사 · EBS 부교재 · 교과서 · 형광펜 독해) → 교재별 페이지 → 상품 상세 → 주문서
- 교재마다 **지문 분석 패키지**(01·02·03)와 **문제 패키지**(05·06·07) 두 가지를 나란히 판매.
  한쪽을 보면 반대쪽을 권해 줍니다
- **장바구니** — 여러 회차를 담아 한 번에 주문, 입금도 한 번.
  같은 교재의 두 갈래를 함께 담으면 묶음 할인이 자동으로 붙습니다
- **부분 · 전체** — 필요한 강만 사실 수 있게 두되, 두 개 이상 담으면
  '전체로 바꾸면 얼마 싸집니다' 를 알려 주고 단추 하나로 바꿔 줍니다
- **구매자 표시(워터마크)** — 받는 그 자리에서 PDF 모든 쪽에 구매자 이름·이메일·주문번호를
  새겨 보냅니다. 원본은 그대로 두고, 무료 자료에는 새기지 않습니다
- **단골 할인** — 이 사이트에서 몇 번째 사시는지로 자동 할인 (이메일이 열쇠, 회원가입 없음)
- **무료 자료실** — 회차마다 뿌리는 한줄해석 · 한줄영어 · 좌지문우해석은 회원가입 없이 바로,
  직독직해처럼 품이 많이 든 자료는 이메일만 적으면 내려받기.
  받는 화면 아래에 **같은 지문의 유료 자료**를 붙여 둡니다
- **자료 요청** — 찾는 교재 이름만 적어 두면, 만든 뒤 연락. **지문을 안 받아도 됩니다**
- **시험지 나눔** — 학교 시험지를 보내면 할인 쿠폰 지급 (동형 모의고사 제작용 자료 수집)
- 라인업의 자료마다 **샘플 PDF** · 공지/업데이트 일정 · 프리패스(무제한 이용권) 사전 신청
- **이용 안내** — 환불 규정(청약철회) · 자료 이용 범위 · 개인정보 처리방침을
  전자상거래법·개인정보보호법 기준 표준 문안으로 채워 둠 (사업자 정보만 바꾸면 됨)
- 화면 전체 **나눔스퀘어라운드** (저장소의 폰트 파일을 직접 제공, CDN 의존 없음)

**관리자 화면** — 코딩·파일 편집 없이 클릭으로 운영합니다.
- 오늘 할 일 대시보드 (입금 확인 / 발송 / 문의 답변 / 시험지 검토 건수)
- 주문·문의 처리, 시험지 검토 → **승인 시 쿠폰 자동 발급·메일 발송**
- **입금 확인 → '자료 보내기' 한 번으로 다운로드 링크 발급·메일 발송**
  (기한 30일 · 20회 제한 · 유출 시 차단 가능. 파일 첨부가 필요 없습니다)
- **매출·세금** — 월별 공급가액/부가세 집계, 세금계산서·현금영수증 발행 대기 목록
- 상품·교재·분류·공지·**오르티카잉 라인업** 등록과 수정 — 저장하면 고객 화면에 바로 반영
- **무료 자료실** 올리기 — 받는 방법(그냥/이메일)은 버튼으로 고르고,
  직독직해를 고르면 이메일 쪽을 자동으로 권합니다. **이메일 명단**은 엑셀로 내려받기
- **검색 등록** — 네이버·구글이 주는 확인 코드를 붙여 넣기만 하면 됩니다
  (meta 태그를 통째로 붙여 넣어도 코드만 뽑아냅니다). sitemap·robots·구조화 데이터는 이미 들어 있음
- 가게 정보(계좌·사업자·프리패스 가격·쿠폰 조건), 설정 백업/복원

결제는 계좌이체 안내 방식입니다. 상품·주문서·관리자 구조는 그대로 두고
나중에 카드 결제만 끼울 수 있습니다.

운영 방법은 **[STORE.md](STORE.md)** 에 정리해 두었습니다.
