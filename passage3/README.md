...
영어 지문(PDF·HWP·이미지·txt)을 넣으면 **한 지문당 최대 4형식**(한줄해석·한줄영어·좌지문우해석·직독직해)의 PDF를 만듭니다.
직독직해는 문장을 '구~절 사이' 의미 단위(청크)로 끊고 각 청크 아래에 우리말 뜻을 답니다(AI, API 키 필요; JSON에 저장돼 재생성은 무료).
...
## 빠른 설치 (권장)
한 번에 라이브러리·Chromium을 설치합니다. (한글 폰트 나눔스퀘어라운드는
PDF에 base64로 임베드되어 별도 설치가 필요 없습니다.)
...
## 웹앱 (수동 설치)
    ...
    # 한글 폰트: 나눔스퀘어라운드가 fonts/ 에 포함되어 PDF에 임베드됩니다(설치 불필요).
    cd passage3
    python webapp.py
    # 브라우저에서 http://localhost:5000
...
5. AI 번역 키(선택) — **영어만 있는 자료**에 한글 해석을 자동으로 채우려면 Claude
   API 키를 붙여넣기. 비우면 해석칸은 빈 채로(해석이 이미 있는 자료는 키 불필요)
6. 생성 & 다운로드 — 형식 1개면 PDF, 여러 개면 zip으로 묶어 다운로드
...
- txt → 그대로
- **분석 JSON(.json)** → 이미 분석된 결과 재입력. 재분석(번역·어휘) 없이 바로 재생성

## 분석 결과 재사용 (제목만 바꿔 재생성, API 비용 없음)
API로 한 번 분석하면 결과와 함께 `(지문명)_ORTICA.json` 이 만들어진다.
- 웹앱: "7. 분석 데이터(JSON) 함께 받기"가 켜져 있으면 zip에 JSON이 포함된다.
- 나중에 **제목만 바꾸려면**: 그 JSON의 `title`(지문주제)을 수정하거나, 웹앱에서
  파일명·문항 시작 번호를 바꾼 뒤 **JSON을 다시 업로드**한다 → 번역·어휘 재분석
  없이 즉시 PDF가 다시 만들어진다(API 호출 0).
