...
DISTRACTOR_RULE = (
    "[선지 구성(오답 출제) 원리] 정답은 지문의 핵심 주장(주제문·반복 어구·역접 But/However 뒤 "
    "필자 주장)을 '지문 단어 그대로가 아니라 다른 말로 재진술'하라(단어 복붙 금지). 오답 4개는 "
    "평가원식 5개 축을 활용하되, 반드시 2개는 '주제와 무관'(축③·④·⑤), 2개는 '주제와 모순/범위 "
    "어긋남'(축①·②)으로 구성하라: "
    "①범위 비틀기(지문보다 너무 넓게 확대 또는 예시·세부로 너무 좁게) "
    "②방향 반전(필자 주장과 정반대·왜곡) "
    "③표면 어휘 함정(눈에 띄는 지문 단어를 그대로 복사해 익숙해 보이나 논점 아님) "
    "④근거 없음(그럴듯하나 지문에 언급·근거 없음) "
    "⑤초점 이동(소재는 맞지만 필자 논점이 아닌 곁가지). "
    "오답 중 '정확히 1개'는 정답과 매우 비슷하되 범위나 방향이 '한 끗' 어긋난 '매력적 오답'으로 "
    "만들어 변별하라. 그 결과 정답은 오직 1개만 성립하고 오답 4개는 확실히 틀리게 하라."
)
# 위 원리(정답=유의어 패러프레이즈)를 적용하는 유형(모두 '가장 적절한 것' 고르기형)
_PARAPHRASE_TYPES = {"main_point", "topic", "title", "implied_meaning", "blank_single"}

# 빈칸추론 전용 오답/정답 원리 — 정답은 '유의어 위주', 오답은 '정답 단어를 재사용하되 논리가 다르게'.
BLANK_RULE = (
    "[빈칸 출제원리] 지문의 핵심 논지가 걸리는 결론부(또는 핵심 명제)를 '____'로 비운다. "
    "정답은 빈칸에 들어갈 말을 '유의어 위주로 재진술'하라 — 지문 표현을 그대로 베끼지 말고 "
    "'같은 뜻의 다른(유의어) 표현'으로 바꿔라. 오답 4개는 '정답 선지에 쓰인 핵심 단어를 1~2개 "
    "그대로 가져다 쓰되, 논리(인과·범위·방향·주어↔목적 관계)를 어긋나게' 만들어라 — 익숙한 단어라 "
    "그럴듯해 보이지만 빈칸 앞뒤 논리와는 맞지 않게 하라. 오답 4개 중 2개는 '주제와 무관한 논리', "
    "2개는 '주제와 모순되는 논리'로 하라. 그 결과 정답은 오직 1개만 성립하게 하라."
)
...
def _variant_hint(ctx: GenContext) -> str:
    ...
    return (f"\n[회차 변형 — {ctx.variant}회차] 같은 지문으로 만드는 여러 회차 중 이 회차용이다. "
            "다른 회차 문항과 '절대 겹치지 않도록' 묻는 지점을 완전히 달리하라: 빈칸·밑줄 위치, "
            "정답이 되는 지점, 초점(핵심) 문장, 선지 구성과 오답 함정을 모두 새로 짜고 표현도 "
            "바꿔라. (지문·유형·출제원리는 그대로 유지하되, 문항 내용은 회차마다 반드시 달라야 한다)")
