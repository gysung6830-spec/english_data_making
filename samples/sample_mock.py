...
def mock_report(title: str = "The Value of Curiosity", source: str = "Mock Reader 2024") -> schemas.Report:
    return schemas.Report(
        ...
        exam=schemas.ExamSection(items=[
            schemas.ExamItem(question_type="주제 파악",
                             content="글의 핵심 주장(호기심의 교육적 가치)을 묻는 문제",
                             tip="'호기심=학습 원동력' 논리를 그대로 정답 근거로 사용."),
            schemas.ExamItem(question_type="빈칸 추론",
                             content="essential / encourage 같은 핵심어를 빈칸으로 출제",
                             tip="유의어(vital, promote)로 변형될 수 있으니 함께 암기."),
            schemas.ExamItem(question_type="어법(문법)",
                             content="to부정사의 부사적 용법, that 명사절 어법 판단",
                             tip="③번 핵심 문법 1·2번을 그대로 변형 출제 가능."),
            schemas.ExamItem(question_type="내용 일치",
                             content="연구 결과(기억 지속) 관련 세부 정보 일치 여부",
                             tip="비교급 longer 를 최상급으로 바꾼 오답에 주의."),
            schemas.ExamItem(question_type="서술형",
                             content="호기심이 학습에 도움이 되는 이유를 영어로 서술",
