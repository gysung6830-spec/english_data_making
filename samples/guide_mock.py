...
def mock_guide() -> Guide:
    chapters = [
        Chapter(
            ...
            core_tip="코드 앞뒤 두 명사에 'A(원인)→B(결과)' 화살표를 직접 그려라. 나머지 수식어를 다 지워도 인과 한 줄이 뼈대다.",
            infer_tip="코드 뜻을 몰라도, 앞이 '행동/조건'이고 뒤가 '변화/결과'로 보이면 앞이 원인일 확률이 높다. 상식적 선후관계로 검증하라.",
            cards=[_code(
        ...
        Chapter(
            ...
            core_tip="부정어에 동그라미 치고 문장 끝에 (−) 표시. 필자의 진짜 주장은 대개 이 부정 뒤에 온다.",
            infer_tip="모르는 단어라도 hardly/rarely/no/lack 부정 신호만 잡으면 '그건 아니다'라는 극성은 확정된다.",
            problems=[
                Problem(
                    kind="mc",
    ...
    return Guide(part0=load_part0(), inference=load_inference(), chapters=chapters,
                 abstract=load_abstract(), part2=load_part2_workbook())
