...
def mock_lecture_passage(title: str = "Fear and Social Development",
    ...
    overview = Overview(
        ...
        key_grammar=KeyGrammar(
            ...
            explanation=[
                "형태: 'A ... than B' 비교에서 than 뒤 B에 '[[조동사(do/will/be)+주어]]' 순서로 도치가 일어날 수 있어.",
                "이유: 주어가 길거나 문어체·강조일 때, 반복을 피하려고 이렇게 도치·압축해서 써.",
                "생략: than 앞뒤로 겹치는 동사·어구는 [[생략]]돼(반복하지 않음).",
                "복원: than will infants who are not → 'than infants who are not (frequently frightened) will (have)'.",
                "주의: than은 [[접속사]]로도 전치사로도 쓰여 — 뒤에 '절'이 오면 접속사이고, 이때 도치가 보여.",
                "해석 요령: 먼저 '비교 대상 B가 무엇인지'부터 잡고 'B보다 더/덜 ~하다'로 옮겨.",
            ],
    ...
    items = [
        _S(1, raw[0], [("관계사 that","앞의 the role 을 that절이 수식"), ("분사구 including","주어를 부연 설명")],
           ...
           [("Many developmental theorists and researchers,", "많은 발달 이론가들과 연구자들은,"),
            ("including [[those]] studying human as well as nonhuman primate subjects,", "인간뿐 아니라 인간이 아닌 영장류를 연구 대상으로 삼는 [[사람들]]까지 포함해,"),
            ("have recognized the role", "그 역할을 인정해 왔다"),
            ("that fear can [[play]] in a primate's social development.", "두려움이 영장류의 사회적 발달에서 [[할 수 있는]]")],
        ...
        _S(2, raw[1], [("부사절 when/until","시간·조건을 나타내는 종속절"), ("수동태 has been p.p.","아기가 '위로받는' 것")],
           ...
           [("When an infant is frightened", "아기가 겁을 먹으면"),
            ("it always seeks out its mother for protection and safety,", "그 아기는 항상 보호와 안전을 위해 엄마를 찾아 나선다,"),
            ("and all exploratory and play activity stops", "그리고 모든 탐색과 놀이 활동이 멈춘다"),
            ("[[until]] the infant has been sufficiently comforted and reassured", "그 아기가 충분히 위로받고 안심하게 [[될 때까지]]"),
            ("by its attachment object.", "자신의 애착 대상에 의해")],
        ...
        _S(3, raw[2], [("비교급 than","'~보다 더 적다' 비교"), ("than절 도치·생략","than will infants who are not = 자주 겁먹지 않는 아기들")],
           ...
           [("Thus, frequently frightened infants", "따라서 자주 겁을 먹는 아기들은"),
            ...
            ("and fewer opportunities to play", "놀 기회도 더 적을 (가능성이 높다)"),
        ...
        _S(4, raw[3], [("serve to+동사원형","'~하는 역할을 하다'"), ("조건절 if","성향이 유지된다면")],
           [("voluntary", "자발적인"), ("restraint", "억제, 제약"),
            ...
            ("maintain", "유지하다")],
           [("Such voluntary restraints may [[serve to]] slow down", "그러한 자발적인 억제는 [[늦추는 역할을 할 수 있다]]"),
            ("the social development of shy or anxious infants", "소심하거나 불안한 아기들의 사회적 발달을"),
            ("if these tendencies are maintained", "만약 이런 성향이 유지된다면"),
