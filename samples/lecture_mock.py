...
def mock_lecture_passage(title: str = "Fear and Social Development",
    ...
    items = [
        _S(1, raw[0], [("관계사 that","앞의 the role 을 that절이 수식"), ("분사구 including","주어를 부연 설명")],
           ...
           [("두려움이 '연기하는' 역할을 학자들이 인정해 왔다.",
             "play a role in은 '~에서 역할을 하다'라는 숙어야. play를 '연기하다·놀다'로 읽으면 오역. 즉 '학자들이 두려움이 사회성 발달에 하는 역할을 인정해 왔다'는, 글의 주제를 던지는 문장이야."),
            ("those는 앞의 대상들(그것들)을 가리킨다.",
             "여기 those는 '연구하는 사람들(연구자들)'을 가리켜. 사람 얘기야.")]),
        _S(2, raw[1], [("부사절 when/until","시간·조건을 나타내는 종속절"), ("수동태 has been p.p.","아기가 '위로받는' 것")],
           ...
           [("아기가 위로받기 전까지는 탐색·놀이를 멈추지 않는다.",
             "until은 '~할 때까지 (멈춘 상태가) 계속'이야. 안심하기 전까지 계속 멈춰 있다는 뜻이지, 멈추지 않는다는 게 아니야."),
            ("아기가 엄마를 위로하고 안심시킨다.",
             "has been comforted and reassured는 수동태라 아기가 '위로받고 안심하게 되는' 거야. 즉 겁먹으면 안심할 때까지 탐색·놀이가 멈춘다는, 주제를 뒷받침하는 근거 문장이야.")]),
        _S(3, raw[2], [("비교급 than","'~보다 더 적다' 비교"), ("than절 도치·생략","than will infants who are not = 자주 겁먹지 않는 아기들")],
           ...
           [("than 이하는 '놀지 않을 아기들'이라는 뜻이다.",
             "than will infants who are not에는 도치·생략이 있어. who are not = 자주 겁먹지 '않는' 아기들이 비교 대상이야."),
            ("겁을 자주 먹는 아기가 오히려 더 많이 논다.",
             "정반대야. 자주 겁먹는 아기는 그렇지 않은 아기보다 탐색·놀이 시간과 기회가 '더 적어'—앞 문장의 결과를 정리한 거야.")]),
        _S(4, raw[3], [("serve to+동사원형","'~하는 역할을 하다'"), ("조건절 if","성향이 유지된다면")],
           ...
           [("자발적인 봉사가 발달을 돕는다는 내용이다.",
             "serve를 '봉사하다'로 읽으면 오역이야. serve to+동사원형은 '~하는 역할을 하다'. voluntary restraints도 봉사가 아니라 아기가 스스로 탐색·놀이를 접는 자기 제약이야."),
            ("이런 억제는 발달을 오히려 빠르게 한다.",
             "slow down은 '늦추다'야. 즉 스스로 놀이를 접는 태도가 유년기 내내 이어지면 사회성 발달이 '느려질' 수 있다는 결론이야.")]),
    ]
