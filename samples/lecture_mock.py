...
def mock_lecture_passage(title: str = "Fear and Social Development",
    ...
    items = [
        ...
        _S(2, raw[1], [("부사절 when/until","시간·조건을 나타내는 종속절"), ("수동태 has been p.p.","아기가 '위로받는' 것")],
           ...
           [("(역할) 이 문장은 앞의 주제와 무관하게 '새로운 화제를 여는' 문장이다.",
             "새 화제가 아니라, 겁먹으면 엄마를 찾고 탐색·놀이가 멈춘다는 '아기 행동'으로 앞 주제(두려움이 발달에 역할을 한다)를 뒷받침하는 '근거' 문장이야."),
        ...
        _S(3, raw[2], [],  # ⑤ 핵심문법(비교급 than 도치)과 중복 → ③ 칩에서는 제외(역할 분담)
           ...
           [("(역할) 이 문장은 앞과 반대되는 '새로운 사례를 드는' 문장이다.",
             "새 사례가 아니야. 맨 앞의 Thus(따라서)가 신호 — 앞 문장(겁먹으면 놀이가 멈춘다)에서 이어지는 '결과를 정리'하는 문장이야."),
