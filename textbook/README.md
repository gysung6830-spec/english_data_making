...
> ⚠️ **샘플 콘텐츠 안내**
> 현재 `data.js` 의 문장·해석은 파이프라인이 끝까지 도는지 보여 주기 위한 **예시 문장**이야.
> 명세가 말하는 실제 2023학년도 수능 기출 문장이 아니라 대표 예문으로 채워 둔 것.
> 선생님의 실제 `data.js` 를 붙여 주면 그대로 교체하면 되고, 코드는 스키마만 맞으면 그대로 돈다.
...
- [ ] 실제 2023 수능 기출 문장으로 `data.js` 교체 (샘플 → 실데이터)
