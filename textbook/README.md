...
1-2. **글의 구조 — 해석 전에 예측!** — 통째로 읽은 뒤 **해석하기 전에** 이 글의 구조를
   6유형(통념→반박 / 주장→근거 / 문제→해결 / 비교·대조 / 시간·순서 / 예시→일반화) 중
   하나로 **직접 골라** 본다. 정답 구조(`passage.structure = {type, why}`)는 지문 끝
   답지 뒤에서 공개. (4단계 훈련 중 '글의 구조' 코너)
