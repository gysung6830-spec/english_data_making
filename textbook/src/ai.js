...
- chunks(끊어읽기): 앞에서부터 순서대로 의미 덩어리로 끊고 직독직해(en=영어조각, kor=우리말).
  ★중요★ en 조각들은 '생략(…) 없이' 원문을 앞에서부터 연속으로 자른 실제 토막이어야 해 —
  모든 en 을 순서대로 이어붙이면 원문 문장이 (거의) 그대로 복원돼야 한다(답지에서 원문 위에
  ‘/’로 끊어 보여주기 때문). kor 은 각 en 조각과 1:1로 매칭되게. 조각은 보통 2~5개
  (전치사구·to부정사·접속사·관계사 등 의미 경계에서 끊기).
