...
REL_SIG = [
    ("인과", "cause", r"\b(lead(?:s|ing)? to|led to|result(?:s|ed|ing)? in|bring(?:s|ing)? about|brought about|give(?:s)? rise to|gave rise to|contribute(?:s|d)? to|stem(?:s|med)? from|arise(?:s)? from|arose from|derive(?:s|d)? from|result(?:s|ed|ing)? from|owing to|due to|thereby|responsible for|account(?:s|ed)? for|drive(?:s|n)?|drove|shape(?:s|d)?|determine(?:s|d)?|be based on|is based on|are based on|rooted in|drive|shape|determine|affect|enable|fuel|foster|promote|trigger|prompt|generate|induce|spark|facilitate|turn into|be caused by|correlate with|tied to|give rise to|boil down to)\b"),
    ("등호", "eq", r"\b(reflect(?:s|ed)?|mirror(?:s|ed)?|represent(?:s|ed)?|embod(?:y|ies|ied)|illustrate(?:s|d)?|exemplif(?:y|ies|ied)|amount(?:s|ed)? to|is equivalent to|are equivalent to|define(?:s|d)?|is defined as|be defined as|known as|referred to as|serve(?:s|d)? as|act(?:s|ed)? as|namely|indicate|imply|symbolize|signify|constitute|associated with|related to|see as|view as|treat as|perceive as|think of as|regard as|liken to|correspond to|stand for|translate into|known as)\b"),
    ("대조", "contr", r"\b(unlike|whereas|in contrast|by contrast|on the other hand|on the contrary|contrary to|differ(?:s|ed)? from|different from|distinct from|distinguish(?:es|ed)?|as opposed to|opposed to|conversely|in comparison|compared (?:to|with)|distinguish from|contrast with|differ|counter|versus|opposite of|in contrast to|stand in opposition|set apart from|diverge from)\b"),
    ("비교", "comp", r"\b(outweigh(?:s|ed)?|surpass(?:es|ed)?|exceed(?:s|ed)?|prevail(?:s|ed)? over|superior to|inferior to|outnumber(?:s|ed)?|outperform(?:s|ed)?|greater than|less than|more than|rather than|instead of|prefer(?:s|red)?|go beyond|no better than|no less than|outrank|fall short of|keep pace with)\b"),
    ("대체", "repl", r"\b(replace(?:s|d)?|displace(?:s|d)?|substitute(?:s|d)? for|give(?:s)? way to|gave way to|switch(?:es|ed)? to|shift(?:s|ed)? from|transition(?:s|ed)? from|substitute|stand in for|take the place of)\b"),
]


POL_POS = r"\b(critical|crucial|essential|vital|fundamental|indispensable|significant|beneficial|benefit(?:s)?|valuable|invaluable|priceless|advantage(?:s|ous)?|useful|powerful|effective|strength(?:s)?|thrive[sd]?|flourish(?:es|ed)?|promising|emphasi[sz]e[sd]?|enhance[sd]?|reinforce[sd]?|prioriti[sz]e[sd]?|prominent|paramount|pivotal|important(?:s|es|ed|ing|ly)?|successful(?:s|es|ed|ing|ly)?|accurate(?:s|d|ed|ing|ly)?|necessar(?:y|ies|ied|ily)|success(?:s|es|ed|ing|ly)?|enrich(?:s|es|ed|ing|ly)?|favorable(?:s|d|ed|ing|ly)?|remarkable(?:s|d|ed|ing|ly)?|rich(?:s|es|ed|ing|ly)?|succeed(?:s|es|ed|ing|ly)?|achievement(?:s|es|ed|ing|ly)?|appreciation(?:s|es|ed|ing|ly)?|attraction(?:s|es|ed|ing|ly)?|attractive(?:s|d|ed|ing|ly)?|better(?:s|es|ed|ing|ly)?|clever(?:s|es|ed|ing|ly)?|extraordinar(?:y|ies|ied|ily)|facilitate(?:s|d|ed|ing|ly)?|freedom(?:s|es|ed|ing|ly)?|good(?:s|es|ed|ing|ly)?|great(?:s|es|ed|ing|ly)?|helpful(?:s|es|ed|ing|ly)?|importance(?:s|d|ed|ing|ly)?|innovation(?:s|es|ed|ing|ly)?|persuasive(?:s|d|ed|ing|ly)?|praise(?:s|d|ed|ing|ly)?|profitable(?:s|d|ed|ing|ly)?|promote(?:s|d|ed|ing|ly)?|reward(?:s|es|ed|ing|ly)?|successfull(?:y|ies|ied|ily)|unique(?:s|d|ed|ing|ly)?|accuratel(?:y|ies|ied|ily)|admiration(?:s|es|ed|ing|ly)?|advanced(?:s|es|ed|ing|ly)?|advancement(?:s|es|ed|ing|ly)?|beautiful(?:s|es|ed|ing|ly)?|beaut(?:y|ies|ied|ily)|best(?:s|es|ed|ing|ly)?|breakthrough(?:s|es|ed|ing|ly)?|brilliant(?:s|es|ed|ing|ly)?|celebrated(?:s|es|ed|ing|ly)?|coherent(?:s|es|ed|ing|ly)?|cohesive(?:s|d|ed|ing|ly)?|competentl(?:y|ies|ied|ily)|confidentl(?:y|ies|ied|ily)|constructive(?:s|d|ed|ing|ly)?|convenience(?:s|d|ed|ing|ly)?|convenient(?:s|es|ed|ing|ly)?|cooperativel(?:y|ies|ied|ily)|creativit(?:y|ies|ied|ily)|credible(?:s|d|ed|ing|ly)?|cultivate(?:s|d|ed|ing|ly)?|delightful(?:s|es|ed|ing|ly)?|diverse(?:s|d|ed|ing|ly)?|eager(?:s|es|ed|ing|ly)?|effectiveness(?:s|es|ed|ing|ly)?|efficient(?:s|es|ed|ing|ly)?|efficientl(?:y|ies|ied|ily)|elevated(?:s|es|ed|ing|ly)?|enable(?:s|d|ed|ing|ly)?|encourage(?:s|d|ed|ing|ly)?|enjoy(?:s|es|ed|ing|ly)?|enjoyment(?:s|es|ed|ing|ly)?|enlightened(?:s|es|ed|ing|ly)?|ensure(?:s|d|ed|ing|ly)?|excellence(?:s|d|ed|ing|ly)?|excellent(?:s|es|ed|ing|ly)?|exceptional(?:s|es|ed|ing|ly)?|expertise(?:s|d|ed|ing|ly)?|fairness(?:s|es|ed|ing|ly)?|fascinating(?:s|es|ed|ing|ly)?|finer(?:s|es|ed|ing|ly)?|flexibilit(?:y|ies|ied|ily)|flourishing(?:s|es|ed|ing|ly)?|gratif(?:y|ies|ied|ily)|greatl(?:y|ies|ied|ily)|harmon(?:y|ies|ied|ily)|health(?:y|ies|ied|ily)|honest(?:y|ies|ied|ily)|impartial(?:s|es|ed|ing|ly)?|impressive(?:s|d|ed|ing|ly)?|improved(?:s|es|ed|ing|ly)?|influential(?:s|es|ed|ing|ly)?|insight(?:s|es|ed|ing|ly)?|insightful(?:s|es|ed|ing|ly)?|inspiring(?:s|es|ed|ing|ly)?|intelligent(?:s|es|ed|ing|ly)?|interesting(?:s|es|ed|ing|ly)?|inventiveness(?:s|es|ed|ing|ly)?|keystone(?:s|d|ed|ing|ly)?|kindness(?:s|es|ed|ing|ly)?|leadership(?:s|es|ed|ing|ly)?|liberation(?:s|es|ed|ing|ly)?|loyalt(?:y|ies|ied|ily)|lucrative(?:s|d|ed|ing|ly)?|maximize(?:s|d|ed|ing|ly)?|meaningful(?:s|es|ed|ing|ly)?|necessit(?:y|ies|ied|ily)|notable(?:s|d|ed|ing|ly)?|opportunit(?:y|ies|ied|ily)|optimal(?:s|es|ed|ing|ly)?|optimize(?:s|d|ed|ing|ly)?|overcome(?:s|d|ed|ing|ly)?|passionate(?:s|d|ed|ing|ly)?|perfect(?:s|es|ed|ing|ly)?|pleasant(?:s|es|ed|ing|ly)?|positive(?:s|d|ed|ing|ly)?|preferred(?:s|es|ed|ing|ly)?|pride(?:s|d|ed|ing|ly)?|productive(?:s|d|ed|ing|ly)?|progress(?:s|es|ed|ing|ly)?|prosperous(?:s|es|ed|ing|ly)?|protect(?:s|es|ed|ing|ly)?|qualit(?:y|ies|ied|ily)|reassuring(?:s|es|ed|ing|ly)?|reliable(?:s|d|ed|ing|ly)?|resonate(?:s|d|ed|ing|ly)?|respect(?:s|es|ed|ing|ly)?|safe(?:s|d|ed|ing|ly)?|safer(?:s|es|ed|ing|ly)?|safet(?:y|ies|ied|ily)|seamless(?:s|es|ed|ing|ly)?|secure(?:s|d|ed|ing|ly)?|skill(?:s|es|ed|ing|ly)?|skilled(?:s|es|ed|ing|ly)?|solid(?:s|es|ed|ing|ly)?|spectacular(?:s|es|ed|ing|ly)?|stimulating(?:s|es|ed|ing|ly)?|striking(?:s|es|ed|ing|ly)?|strong(?:s|es|ed|ing|ly)?|synergistic(?:s|es|ed|ing|ly)?|talented(?:s|es|ed|ing|ly)?|trustworth(?:y|ies|ied|ily)|unparalleled(?:s|es|ed|ing|ly)?|well\-being(?:s|es|ed|ing|ly)?|well\-developed(?:s|es|ed|ing|ly)?|well\-intentioned(?:s|es|ed|ing|ly)?|workable(?:s|d|ed|ing|ly)?|worthwhile(?:s|d|ed|ing|ly)?)\b"
POL_NEG = r"\b(abandon(?:s|ed)?|discard(?:s|ed)?|eliminate[sd]?|neglect(?:s|ed)?|ignore[sd]?|dismiss(?:es|ed)?|overlook(?:s|ed)?|reject(?:s|ed)?|refuse[sd]?|den(?:y|ies|ied)|forbid(?:s|den)?|hinder(?:s|ed)?|diminish(?:es|ed)?|obscure[sd]?|undermine[sd]?|disregard(?:s|ed)?|lack(?:s|ed|ing)?|absence|flaw(?:s|ed)?|drawback|fail(?:s|ed|ing)?|problem(?:s|atic)?|threat(?:s|en(?:s|ed)?)?|harm(?:s|ed|ful)?|damage[sd]?|weaken(?:s|ed)?|suffer(?:s|ed|ing)?|danger(?:ous)?|useless|worthless|destroy(?:s|ed)?|hardly|rarely|scarcely|by no means|difficult(?:s|es|ed|ing|ly)?|struggle(?:s|d|ed|ing|ly)?|conflict(?:s|es|ed|ing|ly)?|decline(?:s|d|ed|ing|ly)?|loss(?:s|es|ed|ing|ly)?|misleading(?:s|es|ed|ing|ly)?|poor(?:s|es|ed|ing|ly)?|risk(?:s|es|ed|ing|ly)?|costl(?:y|ies|ied|ily)|crisis(?:s|es|ed|ing|ly)?|damage(?:s|d|ed|ing|ly)?|harmful(?:s|es|ed|ing|ly)?|irrelevant(?:s|es|ed|ing|ly)?|limited(?:s|es|ed|ing|ly)?|negative(?:s|d|ed|ing|ly)?|worst(?:s|es|ed|ing|ly)?|bias(?:s|es|ed|ing|ly)?|burnout(?:s|es|ed|ing|ly)?|challenge(?:s|d|ed|ing|ly)?|constrain(?:s|es|ed|ing|ly)?|constraint(?:s|es|ed|ing|ly)?|controversial(?:s|es|ed|ing|ly)?|criticize(?:s|d|ed|ing|ly)?|difficult(?:y|ies|ied|ily)|error(?:s|es|ed|ing|ly)?|exclusion(?:s|es|ed|ing|ly)?|fear(?:s|es|ed|ing|ly)?|frustration(?:s|es|ed|ing|ly)?|guilt(?:s|es|ed|ing|ly)?|illusion(?:s|es|ed|ing|ly)?|impossible(?:s|d|ed|ing|ly)?|lose(?:s|d|ed|ing|ly)?|misperception(?:s|es|ed|ing|ly)?|mistake(?:s|d|ed|ing|ly)?|obsession(?:s|es|ed|ing|ly)?|restricted(?:s|es|ed|ing|ly)?|stressful(?:s|es|ed|ing|ly)?|superficial(?:s|es|ed|ing|ly)?|threaten(?:s|es|ed|ing|ly)?|time\-consuming(?:s|es|ed|ing|ly)?|uncertaint(?:y|ies|ied|ily)|uncomfortable(?:s|d|ed|ing|ly)?|upset(?:s|es|ed|ing|ly)?|victim(?:s|es|ed|ing|ly)?|violate(?:s|d|ed|ing|ly)?|vulnerable(?:s|d|ed|ing|ly)?|absent(?:s|es|ed|ing|ly)?|accident(?:s|es|ed|ing|ly)?|adverse(?:s|d|ed|ing|ly)?|aggressive(?:s|d|ed|ing|ly)?|alien(?:s|es|ed|ing|ly)?|ambiguous(?:s|es|ed|ing|ly)?|antithetical(?:s|es|ed|ing|ly)?|anxiet(?:y|ies|ied|ily)|anxiety\-provoking(?:s|es|ed|ing|ly)?|ashamed(?:s|es|ed|ing|ly)?|awkward(?:s|es|ed|ing|ly)?|barrier(?:s|es|ed|ing|ly)?|betrayal(?:s|es|ed|ing|ly)?|biased(?:s|es|ed|ing|ly)?|bizarre(?:s|d|ed|ing|ly)?|block(?:s|es|ed|ing|ly)?|broken(?:s|es|ed|ing|ly)?|bug(?:s|es|ed|ing|ly)?|burden(?:s|es|ed|ing|ly)?|catastrophe(?:s|d|ed|ing|ly)?|challenging(?:s|es|ed|ing|ly)?|chaos(?:s|es|ed|ing|ly)?|complain(?:s|es|ed|ing|ly)?|complicated(?:s|es|ed|ing|ly)?|conflicted(?:s|es|ed|ing|ly)?|conflicting(?:s|es|ed|ing|ly)?|confusing(?:s|es|ed|ing|ly)?|confusion(?:s|es|ed|ing|ly)?|contaminant(?:s|es|ed|ing|ly)?|contaminate(?:s|d|ed|ing|ly)?|contentious(?:s|es|ed|ing|ly)?|controvers(?:y|ies|ied|ily)|dangerous(?:s|es|ed|ing|ly)?|deaf(?:s|es|ed|ing|ly)?|debilitated(?:s|es|ed|ing|ly)?|defeat(?:s|es|ed|ing|ly)?|degradation(?:s|es|ed|ing|ly)?|deteriorate(?:s|d|ed|ing|ly)?|deterioration(?:s|es|ed|ing|ly)?|devalue(?:s|d|ed|ing|ly)?|diminishing(?:s|es|ed|ing|ly)?|disappoint(?:s|es|ed|ing|ly)?|disappointed(?:s|es|ed|ing|ly)?|disaster(?:s|es|ed|ing|ly)?|discomfort(?:s|es|ed|ing|ly)?|disconnect(?:s|es|ed|ing|ly)?|disease(?:s|d|ed|ing|ly)?|disincentive(?:s|d|ed|ing|ly)?|dismissal(?:s|es|ed|ing|ly)?|disorganized(?:s|es|ed|ing|ly)?|disorient(?:s|es|ed|ing|ly)?|distorted(?:s|es|ed|ing|ly)?|distraction(?:s|es|ed|ing|ly)?|distress(?:s|es|ed|ing|ly)?|disturbance(?:s|d|ed|ing|ly)?|disturbing(?:s|es|ed|ing|ly)?|effortful(?:s|es|ed|ing|ly)?|egregious(?:s|es|ed|ing|ly)?|embarrassed(?:s|es|ed|ing|ly)?|embarrassing(?:s|es|ed|ing|ly)?|emergenc(?:y|ies|ied|ily)|envious(?:s|es|ed|ing|ly)?|evade(?:s|d|ed|ing|ly)?|excessive(?:s|d|ed|ing|ly)?|exhausting(?:s|es|ed|ing|ly)?|exploitation(?:s|es|ed|ing|ly)?|failure(?:s|d|ed|ing|ly)?|fallacious(?:s|es|ed|ing|ly)?|fearful(?:s|es|ed|ing|ly)?|flawed(?:s|es|ed|ing|ly)?|forgotten(?:s|es|ed|ing|ly)?|fraud(?:s|es|ed|ing|ly)?|frightening(?:s|es|ed|ing|ly)?|frustrate(?:s|d|ed|ing|ly)?|frustrated(?:s|es|ed|ing|ly)?|harsh(?:s|es|ed|ing|ly)?|hazard(?:s|es|ed|ing|ly)?|high\-risk(?:s|es|ed|ing|ly)?|hurdle(?:s|d|ed|ing|ly)?|ignorance(?:s|d|ed|ing|ly)?|imbalance(?:s|d|ed|ing|ly)?|immoral(?:s|es|ed|ing|ly)?|impair(?:s|es|ed|ing|ly)?|imperceptible(?:s|d|ed|ing|ly)?|imperfect(?:s|es|ed|ing|ly)?|improbable(?:s|d|ed|ing|ly)?|inaccurac(?:y|ies|ied|ily)|inaccurate(?:s|d|ed|ing|ly)?|inaccuratel(?:y|ies|ied|ily)|inaction(?:s|es|ed|ing|ly)?|incoherent(?:s|es|ed|ing|ly)?|incomplete(?:s|d|ed|ing|ly)?|incorrect(?:s|es|ed|ing|ly)?|inefficient(?:s|es|ed|ing|ly)?|inequalit(?:y|ies|ied|ily)|inhibit(?:s|es|ed|ing|ly)?|injured(?:s|es|ed|ing|ly)?|injur(?:y|ies|ied|ily)|insensitivit(?:y|ies|ied|ily)|insignificant(?:s|es|ed|ing|ly)?|insufficient(?:s|es|ed|ing|ly)?|interfere(?:s|d|ed|ing|ly)?|involuntar(?:y|ies|ied|ily)|irrational(?:s|es|ed|ing|ly)?|irreconcilable(?:s|d|ed|ing|ly)?|irritating(?:s|es|ed|ing|ly)?|lacking(?:s|es|ed|ing|ly)?|lost(?:s|es|ed|ing|ly)?|malware(?:s|d|ed|ing|ly)?|mess(?:s|es|ed|ing|ly)?|misidentification(?:s|es|ed|ing|ly)?|misinformation(?:s|es|ed|ing|ly)?|misinterpret(?:s|es|ed|ing|ly)?|misinterpretation(?:s|es|ed|ing|ly)?|misquote(?:s|d|ed|ing|ly)?|misshapen(?:s|es|ed|ing|ly)?|missing(?:s|es|ed|ing|ly)?|misunderstanding(?:s|es|ed|ing|ly)?|myth(?:s|es|ed|ing|ly)?|nervous(?:s|es|ed|ing|ly)?|objection(?:s|es|ed|ing|ly)?|obstacle(?:s|d|ed|ing|ly)?|oppositional(?:s|es|ed|ing|ly)?|outrage(?:s|d|ed|ing|ly)?|overhype(?:s|d|ed|ing|ly)?|overlooked(?:s|es|ed|ing|ly)?|overuse(?:s|d|ed|ing|ly)?|pain(?:s|es|ed|ing|ly)?|panicked(?:s|es|ed|ing|ly)?|paradox(?:s|es|ed|ing|ly)?|paranoid(?:s|es|ed|ing|ly)?|passivit(?:y|ies|ied|ily)|polluted(?:s|es|ed|ing|ly)?|povert(?:y|ies|ied|ily)|pressure(?:s|d|ed|ing|ly)?|problematic(?:s|es|ed|ing|ly)?|regressed(?:s|es|ed|ing|ly)?|reluctance(?:s|d|ed|ing|ly)?|restrict(?:s|es|ed|ing|ly)?|restriction(?:s|es|ed|ing|ly)?|sad(?:s|es|ed|ing|ly)?|scar(?:y|ies|ied|ily)|scattered(?:s|es|ed|ing|ly)?|seldom(?:s|es|ed|ing|ly)?|self\-centered(?:s|es|ed|ing|ly)?|severe(?:s|d|ed|ing|ly)?|shame(?:s|d|ed|ing|ly)?|shocked(?:s|es|ed|ing|ly)?|sorrow(?:s|es|ed|ing|ly)?|stereotype(?:s|d|ed|ing|ly)?|succumb(?:s|es|ed|ing|ly)?|suppress(?:s|es|ed|ing|ly)?|surprised(?:s|es|ed|ing|ly)?|suspicious(?:s|es|ed|ing|ly)?|taint(?:s|es|ed|ing|ly)?|threatening(?:s|es|ed|ing|ly)?|tiresome(?:s|d|ed|ing|ly)?|tiring(?:s|es|ed|ing|ly)?|toxicit(?:y|ies|ied|ily)|unanswered(?:s|es|ed|ing|ly)?|uncertain(?:s|es|ed|ing|ly)?|underestimate(?:s|d|ed|ing|ly)?|undesirable(?:s|d|ed|ing|ly)?|uneasiness(?:s|es|ed|ing|ly)?|unemployment(?:s|es|ed|ing|ly)?|unfamiliar(?:s|es|ed|ing|ly)?|unfortunatel(?:y|ies|ied|ily)|unglamorous(?:s|es|ed|ing|ly)?|unhealth(?:y|ies|ied|ily)|unintended(?:s|es|ed|ing|ly)?|unlikel(?:y|ies|ied|ily)|unpleasant(?:s|es|ed|ing|ly)?|unpleasantr(?:y|ies|ied|ily)|unpredictable(?:s|d|ed|ing|ly)?|unreliable(?:s|d|ed|ing|ly)?|unremarkable(?:s|d|ed|ing|ly)?|unrul(?:y|ies|ied|ily)|unsustainable(?:s|d|ed|ing|ly)?|vulnerabilit(?:y|ies|ied|ily)|weak(?:s|es|ed|ing|ly)?|wicked(?:s|es|ed|ing|ly)?|worr(?:y|ies|ied|ily)|worse(?:s|d|ed|ing|ly)?|wrong(?:s|es|ed|ing|ly)?)\b"
...
.opt .n{ flex:none; font-weight:800; width:14px; } .opt .txwrap{ flex:1; min-width:0; } .opt .tx{ display:inline; } .opt .jd{ flex:0 1 auto; min-width:0; font-size:8.6px; font-weight:800; }
