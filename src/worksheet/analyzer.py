...
def analyze_prompt(text: str, index: int, strength: str, hints: list[str]) -> str:
    ...
    return (
        ...
        "자연스럽습니다.\n"
        "    · ⚠️ 과도한 의역 금지 — reading_ko 는 그 영어 조각의 '단어·구조를 그대로 살린 직역'입니다. "
        "학생이 '이 영어가 왜 이 한글이 되는지' 바로 알 수 있어야 하므로, 영어에 없는 말을 더하거나 어순을 크게 "
        "바꾸지 말고(영어 어순 유지), 조사·어미 정도만 자연스럽게 붙입니다. 매끄러운 의역은 translation 에 따로 담고, "
        "reading_ko 는 직역체를 유지하세요. (단, 한 조각 안에서 완전히 뜻이 안 통할 만큼 토막 내지는 말 것.)\n"
