...
def analyze_prompt(text: str, index: int, strength: str, hints: list[str]) -> str:
    ...
    return (
        ...
        + hint_block +
        "\n[작성 규칙]\n"
        "- lines: 화면 줄바꿈 단위 배열. 각 줄은 tokens 배열을 가집니다. 한 줄이 너무 길면 "
        "의미 단위로 2~3줄로 나누세요. 짧으면 한 줄이어도 됩니다.\n"
        "- 각 token 의 text 는 원문 단어/구를 '순서대로, 빠짐없이' 담아 이어붙이면 원문이 되어야 합니다. "
        "성분/주석을 붙일 단위로 묶으세요(예: 'the curious students' 를 한 토큰으로).\n"
        "- role: 문장 성분이면 'S','V','O','C' (복문은 'S①','V①','S②'…). 아니면 빈 문자열.\n"
        "- note: 그 토큰의 문법 주석(예: '현재분사','주격 관계대명사','동격 that','to-v(부사적)'). 없으면 빈 문자열.\n"
        "- note_kind: 성분/준동사/연결사=‘lbl’ 또는 ‘blue’, 어법 강조(분사·수동태·관계사·수일치)=‘red’, "
        "해석 힌트(유의어=,반의어↔)=‘gray’.\n"
        "- wrong: 시험 함정 오답형이 있으면 '올바른형/틀린형(X)' 로(예: 'directed/direct(X)'). 없으면 빈 문자열.\n"
        "- above: 생략/복원 메모가 필요하면(예: 'it is 생략'). 없으면 빈 문자열.\n"
        "- hl: 특히 강조할 어구면 'y'(노랑) 또는 'g'(연두). 기본은 빈 문자열.\n"
        "- underline/color: 필요할 때만.\n"
        + _strength_rule(strength) +
        "- translation: 이 문장의 자연스러운 한국어 해석(직독직해체).\n"
        "- badge: 문장 역할이 뚜렷하면 '순/삽','서','예시','결론','주제' 등 짧은 뱃지. 아니면 빈 문자열.\n"
    )
