...
def analyze_prompt(text: str, index: int, strength: str, hints: list[str],
    ...
    return (
        ...
        "  ⭐ [어법성판단(밑줄형) 우선 점검 목록] — 아래는 내신·모의고사 '어법상 틀린 것 고르기'의 단골 "
        "함정입니다. 문장에 해당 요소가 있으면 반드시 red note 로 짚고(위 규칙대로), 실제 시험 함정이면 "
        "오답형(X)까지 병기하세요(text=지문의 올바른 형태, wrong=그 자리에 넣으면 틀리는 형태). "
        "억지로 만들지는 말고, 문장에 실제로 있는 것만 짚습니다:\n"
        "    (a) 정동사 vs 준동사: 한 절엔 본동사가 하나. 이미 본동사가 있으면 그 뒤 동사는 분사/관계절이어야 "
        "함(문장 본동사 자리 판단이 핵심). 예 'a man [carrying] a box walked in' — 정답 carrying(현재분사), "
        "carried(X). note='분사(정동사 자리 아님)'.\n"
        "    (b) 감정분사: 감정을 '유발'하는 대상이면 현재분사(-ing), 감정을 '느끼는' 주체면 과거분사(p.p.). "
        "예 사물 'the result was surprising' / 사람 'he was surprised'. 함정 surprised(X)↔surprising, "
        "boring↔bored. note='감정분사(유발=-ing/느낌=p.p.)'.\n"
        "    (c) 재귀대명사 vs 인칭대명사: 주어와 목적어가 같은 대상이면 재귀대명사(-self). "
        "예 'she taught herself' — herself 정답, her(X). note='재귀대명사'.\n"
        "    (d) 대명사 수·격 일치: 가리키는 명사의 수·격에 맞춤. 단수 it↔복수 them, one↔it, "
        "'that of(단수)'↔'those of(복수)'. 예 '... larger than that of Seoul' — 선행 단수면 those(X). "
        "note='대명사 수일치'.\n"
        "    (e) 부사 vs 형용사: 동사·형용사·부사·문장 수식은 부사, 명사 수식·2형식 보어는 형용사. "
        "예 'remarkably good' — remarkable(X); 'seem happy' — happily(X). note='부사/형용사 구별'.\n"
        "    (f) 접속사 vs 전치사: 뒤에 '절(S+V)'이면 접속사, '명사(구)'면 전치사. "
        "because+절/because of+명사, while+절/during+명사, although+절/despite·in spite of+명사. "
        "예 'despite the rain'—although(X), 'because it rained'—because of(X). note='전치사/접속사 구별'.\n"
        "    (g) 관계사 심화: 선행사 없는 명사절이면 what, 선행사 있으면 that/which; 사람 주격 who; "
        "'전치사+관계대명사'(=관계부사); 뒤 절이 완전하면 관계부사/that, 불완전하면 관계대명사. "
        "예 'the thing that he wants' vs 'what he wants'(선행사 없음). what↔which/that 혼동 주의. "
        "note='관계대명사 what(선행사 포함)' 등.\n"
        "    (h) 가목적어 it: 5형식에서 목적어가 to부정사·that절이면 가목적어 it 필요. "
        "예 'find it hard to focus' — it 생략(X). note='가목적어 it'.\n"
        "    (i) 명사절 접속사: whether(여부·or)↔if, 완전한 절이면 that, 불완전하면 what. "
        "예 주어 자리 'Whether ~ matters' — if(X). note='명사절 whether/that/what'.\n"
        "    (j) 수량·명사 일치: 셀 수 있음 many/(a) few/a number of(+복수), 셀 수 없음 much/(a) little; "
        "'the number of(단수 취급)'↔'a number of(복수)'. 예 'A number of students are' — is(X). "
        "note='수량 표현·수일치'.\n"
        "    (k) 도치·부정: 부정어(Never/Hardly/Not only)가 문두면 조동사+주어 도치; 가정법 도치 "
        "Were I.../Had I known...; 부분부정(not all/always=전부는 아님). 예 'Not only did he ...' — "
        "he did(X). note='부정어 도치' / '가정법 도치'.\n"
        ...
        "자연스럽습니다.\n"
        "    · ⚠️ be동사(계사) 단독 끊기 금지: is/are/was/were/be/been(및 will be·can be·could be·"
        "should be 같은 서술어)을 '보어와 분리'해 홀로 한 조각으로 만들지 마세요. be동사는 뒤의 보어와 "
        "'한 조각'으로 묶어 'X이다/X였다/X일 수 있다'처럼 옮깁니다. 즉 한글 조각이 '이다·이었다·되어야 한다·"
        "이어야 한다·~이다·~일 수 있습니다'처럼 계사만 달랑 남으면 안 됩니다. "
        "예: 'the strongest attractor / is / our belief system'(X) → "
        "'가장 강력한 끌개는 / 우리의 신념 체계이다'(O, be+보어를 한 조각으로).\n"
        "    · ⚠️⚠️ reading_ko 는 '엄격한 직역'입니다(해석 수업용). 반드시 지키세요:\n"
        "        (1) 영어 어순을 그대로 따라간다 — 주어→동사→목적어/보어 순서를 매끄럽게 재배열하지 말 것. "
        "영어 각 조각과 한글 조각이 '1:1'로 떨어져야 한다(같은 위치 = 같은 내용).\n"
        "        (2) 영어에 없는 말을 더하거나 빼지 말고 단어·구조를 그대로 옮긴다(조사·어미만 자연스럽게).\n"
        "        (3) 관용구·숙어는 '직역을 먼저' 쓰고 뜻을 괄호로 병기한다. "
        "예: 'To whom it may concern' → '관계될 수 있는 그 누구에게 (→ 담당자분께)'.\n"
        "        (4) 매끄럽게 다듬은 의역은 translation(전체 해석)에만 담고, reading_ko 는 직역체를 유지한다.\n"
        "      예: 'As we wish to update / the overall design of our brochure, / we are considering / "
        "working with a new printing company.' → reading_ko(4개) "
        "[\"우리가 바꾸고 싶기에\", \"우리 책자의 전반적인 디자인을,\", \"우리는 고려하고 있습니다\", "
        "\"새로운 인쇄 회사와 일하는 것을\"] — 영어 어순 그대로, 재배열 금지. "
        "(⚠️ '~하므로 …을 우리는 고려한다'처럼 목적어를 앞으로 당겨 매끄럽게 만들지 말 것.)\n"
        "      또 다른 예: 영어 3조각 [\"Certain nomadic tribes don't have much,\", \"yet they are happy to share\", "
        "\"because it is in their interest to do so.\"] → reading_ko(3개) "
        "[\"어떤 유목 부족들은 많이 가지고 있지 않다\", \"그러나 그들은 기꺼이 나눈다\", \"왜냐하면 그렇게 하는 것이 그들의 이익이기 때문이다\"].\n"
