"""구문 분석 학습지 자동 생성기 (지문 분석 학습지).

영어 지문을 문장 단위로 쪼개 '직독직해 + 구문 태깅 + 포인트 박스' 학습지를
HTML/PDF 로 출력한다. 두 가지 레이아웃을 지원한다.

- 레이아웃 A (포인트박스형) : 리본 + 좌 구문 분석 / 우 포인트 박스
- 레이아웃 B (직독직해형)   : 주황 헤더바 + 좌 영어 원문·핵심 문법 / 우 직독직해·핵심 단어

파이프라인:
    loader → splitter → analyzer(규칙+LLM) → point_builder → renderer → PDF

★ 프로젝트 경계 (본 프로젝트 = 구문 분석 학습지)
    본 프로젝트 소유 파일  : src/worksheet/**, templates/worksheet_*.html.j2,
                             tests/test_worksheet.py, worksheet_app.py(전용 웹앱).
    별개 도구(수정 금지)   : 6섹션 분석 도구 — src/analyze.py, src/prompts.py,
                             src/schemas.py, templates/report.html.j2, styles.css,
                             webapp.py, tests/test_offline.py.
    공유 코어(둘 다 사용)  : src/extract.py, src/analyze.extract_passages(지문 분리),
                             src/client.py, src/config.py, web_common.py.
"""
from __future__ import annotations

from .models import (Analysis, FlowStep, GrammarChip, KeyWord, LitChunk,
                     LiteralSentence, Point, Sentence, Token, VocabEntry)

__all__ = ["Analysis", "FlowStep", "GrammarChip", "KeyWord", "LitChunk",
           "LiteralSentence", "Point", "Sentence", "Token", "VocabEntry"]
