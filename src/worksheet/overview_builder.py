...
def build_prompt(analysis: Analysis) -> str:
    ...
    return (
        ...
        "[작성 규칙]\n"
        "0) title_en / title_ko: 지문 내용을 대표하는 '짧은 영문 제목'(title_en, 3~7단어)을 짓고, "
        "title_ko 는 그 영문 제목을 '그대로 옮긴 한국어 제목'(같은 뜻)으로 쓰세요. "
        "⚠️ 영문 제목과 한글 제목의 '내용이 반드시 일치'해야 합니다(서로 다른 소재·표현·의미 금지). "
        "예: 'The Hidden Cost of Going Green' → '친환경 전환에 숨은 비용'.\n"
        "1) vocab: 지문 핵심 어휘 8~12개. 각 항목 word(단어/표현), meaning(한글 뜻), "
        "syn(유의어, 없으면 '—'), ant(반의어, 없으면 '—'), sent(등장 문장 번호).\n"
        "   ▸ 선정 우선순위: ①동사 ②형용사 ③부사 위주로, '시험에 나올 만한 핵심 단어'를 "
        "우선순위 높은 순으로 고르세요. 관용구·구동사·연어(collocation)도 좋습니다. "
        "명사는 원칙적으로 제외하되, 그 지문의 '핵심 키워드'인 명사라면 넣어도 됩니다. "
        "쉬운 기초 단어나 지엽적 명사(고유명사·일반 사물 등)는 넣지 마세요.\n"
