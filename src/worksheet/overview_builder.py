...
def build_prompt(analysis: Analysis) -> str:
    ...
    return (
        ...
        "   ⚠️ summary_easy 는 반드시 summary(주제)와 '같은 방향·같은 주장'이어야 합니다. "
        "비유를 쓰더라도 글이 실제로 주장하는 결론과 어긋나거나 반대가 되면 안 됩니다. "
        "예: 글이 '리더는 사람들과 잘 섞여야 한다'를 주장하면 → '사람들과 섞여야 제 역할을 한다(찻잎도 물에 우러나야 맛이 나듯)'처럼 "
        "'섞여야 한다'는 결론이 분명히 드러나게. '안 섞이면 안 우러난다'처럼 방향이 헷갈리거나 주제를 벗어난 예시는 금지.\n"
        "1) vocab: 지문 핵심 어휘 8~12개. 각 항목 word(단어/표현), meaning(한글 뜻), "
        "syn(유의어, 없으면 '—'), ant(반의어, 없으면 '—'), sent(등장 문장 번호).\n"
        "   ▸ 선정 기준(중요): '고등학교 수준(수능·모의고사 빈출) 이상의 어휘'를 고르세요. "
        "중학교 기초 단어나 아주 쉬운 일상어(make, good, help, use, work 등)는 제외합니다. "
        "그중에서도 '유의어·반의어가 뚜렷한' 단어를 최우선으로. 품사는 동사·형용사·부사·명사 모두 대상이며, "
        "고등 수준 명사(추상·개념어, 예: perception, tendency, isolation)도 적극 포함하세요. "
        "관용구·구동사·연어(collocation)도 좋습니다. "
        "다만 고유명사나 지엽적·너무 쉬운 명사(일반 사물 등)는 제외합니다.\n"
        "   ▸ 유의어·반의어 품질(엄격): 아래를 모두 만족할 때만 채우고, 하나라도 어긋나면 '—' 로 두세요.\n"
        "     ① 문맥 일치 — 그 단어가 '이 지문에서 쓰인 뜻'에 맞는 유의어/반의어만. 다의어는 지문 의미 기준"
        "(예: 'address'가 '해결하다'로 쓰였으면 유의어는 tackle·resolve, '주소'가 아님).\n"
        "     ② 같은 품사 — 동사↔동사, 형용사↔형용사, 명사↔명사로 형태·품사를 일치시키고 철자를 정확히.\n"
        "     ③ 확실할 때만 — 뜻이 거의 같은(유의어)·정반대인(반의어) 표준 단어만. 조금이라도 애매하거나 "
        "부분적으로만 겹치면 억지로 넣지 말고 '—'. 한 칸에 1~2개까지만(3개 이상 나열 금지).\n"
        "     ④ 쉬운 표준어 — 고등학생이 알 만한(또는 시험 빈출) 단어로. 희귀어·지나친 문어체·거의 안 쓰는 "
        "동의어는 피하세요.\n"
        "     ⑤ 반의어는 '진짜 반대 개념'일 때만 — 단순히 관련 없는 단어를 반의어로 넣지 마세요"
        "(예: increase↔decrease 는 OK, increase↔stop 은 금지). 없으면 '—'.\n"
        "     ※ 유의어·반의어가 위 기준으로 뚜렷하지 않은 단어라면, 그 단어를 빼고 유의어·반의어가 확실한 "
        "다른 핵심 어휘로 대체하세요('—'만 잔뜩인 목록보다 뚜렷한 8개가 낫습니다).\n"
        ...
        "   ⚠️ easy 도 그 단계 text(논리)의 '방향·주장과 일치'해야 합니다. 비유가 원문 주장과 반대되거나 "
        "주제에서 벗어나지 않게 하세요.\n"
