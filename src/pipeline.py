...
def build_workbook_bundle_for_pdf(client: ClaudeClient, cfg: Config, src: Path):
    ...
    """
    from .textutil import qno_label, format_qno, source_prefix
    ...
    for i, ex in enumerate(_extract_passages_for_pdf(client, cfg, src)):
        ...
        #  · 라벨은 '교재명 + 단원-문항' 형식(예: '올림포스 독해 10-1'). 교재명(출처)을 문항번호 앞에.
        qno = (format_qno(ex.q_no, ex.title, ex.source, ex.topic_ko)
               or qno_label(ex.source) or qno_label(src.name) or f"{i + 1}번")
        pref = source_prefix(ex.source)
        qno = f"{pref} {qno}".strip() if pref else qno
...
def derive_writing_form_packs(writing_packs, packs) -> list:
    ...
    for wp, pp in zip(writing_packs or [], packs or []):
        ...
        fp = writing_render.build_writing_pack(
            ...
            caution=writing_render.WFORM_CAUTION, caution_badge="✎ 어형변형", caution_style="info")
