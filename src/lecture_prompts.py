...
def overview_prompt(title: str, sentences: list[LectureSentence]) -> str:
    ...
    return (
        ...
        "모든 값은 영어로만(한글 절대 금지).\n"
        "- summary(📝 글 내용 정리): 지문 전체 내용을 '필자의 핵심 의견을 중심으로' 정리한 요약"
        "(2~3문장, 한국어). 도입→전개→결론의 흐름이 드러나되, 어려운 내용도 학생이 이해하도록 "
        "쉽게 쓰세요.\n"
    )
...
def sentence_prompt(title: str, sentences: list[LectureSentence]) -> str:
    ...
    return (
        ...
        "만드세요. 모두 '틀린(X)' 진술이어야 합니다(정답은 넣지 마세요). ⚠️ 반드시 '의미 중심'으로만: "
        "그 문장이 글에서 뜻하는 바·주장·역할·논리를 오해한 진술을 쓰세요(예: 인과·반대·비교 방향을 "
        "뒤집거나, 주제를 근거로/근거를 주제로 착각). '단어 번역 실수(어휘·직역)'는 넣지 마세요 — "
        "해석 난이도는 위 끊어읽기 빈칸이 담당합니다. 각 항목: statement(그럴듯하지만 의미가 틀린 진술 "
        "한 줄), why(왜 틀렸는지 + '바른 의미'를 쉬운 말로, 그 문장의 글 속 역할이 드러나게).\n"
    )
