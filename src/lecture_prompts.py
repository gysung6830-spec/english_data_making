...
def sentence_prompt(title: str, sentences: list[LectureSentence]) -> str:
    ...
    return (
        ...
        "- role: 이 문장이 글에서 하는 역할을 짧게(예: '주제 제시', '통념', '반박/전환', '근거', "
        "'예시', '부연', '결과', '결론'). ④ 글 정리의 '문장 흐름'에 쓰입니다.\n"
        ...
        "조각만의 직독직해입니다.\n"
        "  · blank(빈칸 출제원리 = '해석하기 어려운/오역 위험이 큰 부분'): 그 조각이 해석하기 까다롭거나 "
        "오역 위험이 큰 부분(다의어·구조 오인·수식 범위·비교 대상·수동/도치/생략·지칭 등)이면 true, "
        "쉬운 부분은 false. 한 문장에서 '가장 어려운 1~2개 조각'만 true. 학생은 true 인 조각의 ko 를 "
        "직접 써넣어 '해석 난이도'를 여기서 해결합니다(그 조각의 영어가 오역 주의 지점으로 표시됨).\n"
