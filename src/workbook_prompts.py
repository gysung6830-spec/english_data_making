...
1) 동사·준동사(type=verb) → (동사원형)을 주고 문맥에 맞게 어형 변화시킨다.
   ★ 통합 카드에서는 동사·준동사를 '전부' 내지 말고 '핵심만 선별'한다(한 문장에 1~2개 정도).
     시제·태(수동)·수일치·준동사(to부정사/동명사/분사) 판단이 '실제로 헷갈리는' 동사 위주로 고른다.
     be/have 같은 단순 동사나 뻔한 형태는 통합 카드에서는 굳이 내지 않는다.
     ※ 지문의 '모든' 동사는 '어형 변형(form)' 단일 유형 워크시트가 따로 전부 다루므로,
       통합 카드가 동사 문항으로만 가득 차지 않게 한다(다른 유형이 들어갈 자리를 남긴다).
...
[유형을 골고루 — 매우 중요]
- 한 지문 전체가 '동사(verb) 문항만'으로 채워지면 안 된다. 아래 신호를 '문장마다' 훑어 해당 유형을
  반드시 발굴해서 6개 유형이 지문 전체에 골고루 분포하게 한다(동사 편중은 오류로 간주).
- 연결사(conj): and/but/yet/so/for/because/while/although/however/therefore/thus 등 → 논리 흐름 선택으로 출제.
- 특수구문(order): 비교급·최상급, 'the 비교급 …, the 비교급 …', as ~ as, 도치(부정어/so+동사+주어/장소부사구),
  강조구문(It is ~ that), 간접의문문(how/what/why + 주어 + 동사), 가정법 → 어구 배열로 출제.
- 관계사·대명사(rel): who/which/that/what/where/when/whose, 지시대명사(this/that/these/those) → 어법 선택.
- 형용사·부사(adj): 유의어·반의어 대비가 뚜렷한 핵심 형용사/부사 → [원문/유의어/반의어] 로 출제.
- 지칭(ref): 명확한 선행사가 있는 대명사, 앞 문장을 받는 this/that/it → 지칭 대상 고르기.
- 목표: 지문 전체에서 conj·order·rel·adj·ref 가 '각각 최소 1문항 이상'(해당 요소가 있으면) 나오도록 한다.

[균등 분배 — 동사 상한 + 유형별 목표]
- (A) 동사 상한: 동사(verb) 문항 수는 '지문 전체 문항 수의 40%를 넘지 않게' 한다.
  또한 '한 문장에 동사 문항은 최대 2개'까지만 낸다(나머지 동사는 어형 워크시트가 담당).
- (B) 유형별 목표: 아래 유형은 해당 요소가 지문에 있으면 '지문당 약 2문항'을 목표로 고르게 낸다
  — 연결사(conj) · 특수구문(order) · 관계사·대명사(rel) · 형용사·부사(adj) · 지칭(ref).
  (해당 요소가 지문에 아예 없으면 0이어도 된다. 없는 것을 억지로 만들지는 말 것.)
