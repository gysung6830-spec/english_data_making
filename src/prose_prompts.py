...
1) 어법 양자택일(grammar) → display = "[ A / B ]", 정답 하나.
   ★ 그 문장에서 어법 포인트를 '문장당 최소 2개, 최대 5개' 출제한다.
     후보가 5개보다 많으면 아래 우선순위가 높은 것부터 5개만, 2개보다 적으면 최소 2개는 발굴한다
     ...
     ★★★ 오답 B 는 그 문장·문맥에서 '정답이 아니어야' 한다 — 다음 둘 중 하나:
        (a) 문법적으로 비문이거나, (b) 문맥상 명백히 부적절(뜻이 안 맞음)해야 한다.
        ⇒ 금지되는 것은 '정답과 뜻이 같아 실제로 교체 가능한(둘 다 정답이 되는)' 선택지뿐이다.
        - 금지 예(뜻이 같아 교체 가능 → 둘 다 정답):
          · 제한적 관계절의 [ that / which ] (사물 선행사) · [ that / who ] (사람 선행사)
            — that 과 who/which 모두 맞아 교체 가능하므로 금지. (단 [ who / which ]는 선행사가
            사람이면 which 가 비문, 사물이면 who 가 비문이라 '허용'된다.)
          · 유의어끼리 (예: [ big / large ]).
          · 목적어로 to·-ing 를 '둘 다' 취하고 뜻도 같은 동사의 [ to-V / V-ing ]
            (like/love/hate/start/begin/continue …), be동사+형용사 뒤 [ to-V / -ing ] 처럼 둘 다 되는 자리.
        - 허용 예(뜻이 달라 문맥이 정답을 정함, 또는 오답이 비문):
          · 종속접속사 [ although / because ]·[ while / because ]·[ as / when ]·[ after / before ]
            — 뜻이 다르므로 문맥상 하나만 맞음. (단 뜻이 사실상 같은 [ although / though ] 류는 금지.)
          · 전치사 [ on / of ]·[ of / for ]·[ with / to ] — 뜻·역할이 달라 문맥으로 하나가 정해짐.
          · 등위접속사 [ and / but ]·[ and / or ] — 뜻이 달라 문맥으로 하나가 정해짐.
          · [ that / what ](선행사 유무), [ although / despite ](뒤가 절 vs 명사), 조동사 뒤 [ 원형 / -ing ],
            분사 태 [ p.p. / -ing ], as~as의 [ as / so ], nor(neither 없이 비문), among/between(수로 확정).
        - 판정법: 오답 B 를 그 자리에 넣었을 때 '비문이거나 문맥상 틀리다'고 말할 수 있으면 출제 OK.
          '정답과 뜻이 같아 넣어도 자연스럽게 성립'하면(둘 다 정답) 그 포인트는 버리고 다른 포인트를 낸다.
...
3) 어휘 — '난도 하'와 '난도 상' 두 종류를 '모두' 만든다(같은 문장에 대해 각각).
   ★ 각 유형은 '한 문장에 최소 2개, 최대 5개'를 출제한다(어휘가 정말 부족한 아주 짧은 문장만 예외로 1개).
...
4) 대명사 지칭 선택(ref) → 지문 속 대명사·지시어(it, they, them, this, that, those 등)가
   ...
   ★★ 밑줄 대상({{Pn}} 바로 앞)은 '반드시 대명사·지시어'(it/its/they/them/their/this/that/these/those/
     he/his/she/her/one/which/who/such 등)여야 한다. '일반 명사'(예: pests, forests)를 밑줄 대상으로 삼지 말 것 —
     그 명사 자신을 정답으로 주는 '순환 문항'(예: pests = [ weeds and pests / … ])은 절대 금지.
   ★★ 정답·오답 보기에 '그 대명사 자체'를 포함한 구를 넣지 말 것(예: 대상 its → 정답/오답 'its intensity' 금지).
     지칭이 가리키는 것은 '다른 명사구'이므로, 보기의 어떤 항목도 밑줄 친 대명사 단어를 포함해선 안 된다.
