...
1) 어법 양자택일(grammar) → display = "[ A / B ]", 정답 하나.
   ★ 그 문장에서 어법 포인트를 '문장당 최소 2개, 최대 5개' 출제한다.
     ...
     ★★★ '둘 다 문법적으로 맞는' 선택지는 절대 금지한다. 그 문장·문맥에서 정답 A 하나만 맞고
        오답 B 는 '문법적으로 틀려야' 한다(뜻·자연스러움 차이가 아니라 '문법 오류'여야 함).
        - 금지 예(둘 다 성립): 제한적 관계절의 [ that / which ](사물 선행사 — 둘 다 맞음),
          등위접속사 뜻 선택 [ and / but ]·[ and / or ], 종속접속사 뜻 선택 [ although / because ]·
          [ while / because ]·[ as / when ]·[ after / before ], 콜로케이션이 둘 다 가능한 전치사
          [ on / of ]·[ of / for ], 목적어로 to·-ing 둘 다 취하는 동사의 [ to-V / V-ing ]
          (like/love/start/begin/continue …), 뜻만 다른 부사 [ too / so ] 등.
        - 허용 예(오답이 비문): [ that / what ](선행사 유무), [ although / despite ](뒤가 절 vs 명사),
          조동사 뒤 [ 원형 / -ing ], 분사 태 [ p.p. / -ing ], as~as의 [ as / so ], nor(neither 없이 비문),
          among/between(수로 확정), 관계부사 vs 관계대명사(뒤 절의 완전/불완전).
        - 판정법: 오답 B 를 실제로 그 자리에 넣어 문장을 읽었을 때 '문법적으로 틀리다'고 단정할 수
          있어야만 출제한다. 조금이라도 성립하면(둘 다 맞으면) 그 포인트는 버리고 다른 포인트를 낸다.
