...
_MULTI_RULES = (
    ...
    "- ⚠️ 지문 본문은 '처음부터 끝까지 한 문장도 빠뜨리지 말고' 옮기세요(중간 요약·생략 금지). "
    "각 문장은 마침표(.)와 그 뒤 '공백'을 반드시 살려 정확히 구분하고, 원문의 철자·문장부호를 그대로 두세요. "
    "한 문단이 여러 문장이면 그 문장들을 모두 담고, 문단이 바뀌면 paragraphs 항목을 나눠 담으세요."
)


# 영어/한글 배치가 어떻든 영어 원문만 순서대로 뽑도록 하는 공통 지침.
_LAYOUT_RULES = (
    "\n- 지문 배치는 다양할 수 있습니다. 아래 '모든 배치'에서 '영어 원문만' 순서대로 이어 뽑고, "
    "한국어(해석)는 전부 버리세요:\n"
    "  (a) 좌 영어 / 우 한글 2단 대조 → 왼쪽 영어를 위→아래 순서로 이어붙임.\n"
    "  (b) 영어만 → 그대로.\n"
    "  (c) 영어 한 줄 / 한글 한 줄 번갈아 → 영어 줄만 순서대로 이어붙임.\n"
    "- 한 영어 문장이 여러 줄로 잘려 있으면 원래 한 문장으로 자연스럽게 이어붙이고, "
    "문장은 마침표(.)와 공백으로 정확히 구분해 한 문장도 빠뜨리지 마세요."
)
...
def vocab_prompt(title: str, body: str, lo: int, hi: int) -> str:
    return (
        ...
        "- korean_summary: 위 english_summary 를 자연스러운 한국어로 정확히 해석한 문장을 쓰세요.\n\n"
        + _passage_block(title, body)
    )
...
def exam_prompt(title: str, body: str, grammar: schemas.GrammarSection | None,
    ...
    return (
        ...
        "[지칭추론] 지문에 나오는 대명사·지시어(it, this, that, these, those, they 등) 중 "
        "출제될 수 있는 것을 '모두' 찾아, 각 줄에 다음 형식으로 나열하세요:\n"
