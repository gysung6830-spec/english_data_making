...
def ws_compose_prompt(title: str, body: str) -> str:
    return (
        ...
        "'어법 조건 + 단어 수'에 맞춰 영작하게 하는 문제를 만드세요. 각 문항은 '같은 정답 문장'을 "
        "'하·중·상' 세 난이도로 나눠 제시하는데, 난이도는 '주어지는 단어의 개수'로만 구분합니다.\n"
        "- items 3~4개, 각 {korean, conditions, given_low, given_mid, given_high, word_count, answer, explanation}.\n"
        "- korean: 영작해야 할 우리말 문장(지문 문장의 자연스러운 번역).\n"
        "- answer: 지문의 원문 문장(그대로). 정답은 반드시 지문에 있는 문장이어야 합니다. "
        "세 난이도 모두 정답은 이 문장 하나로 같습니다.\n"
        "- conditions: 지켜야 할 어법 조건 목록(예: '관계대명사 who 사용', '분사구문으로', '비교급 사용'). "
        "그 문장에 실제로 쓰인 문법을 조건으로 제시하세요.\n"
        "- ★ 난이도별 보기 단어(모두 원형으로 제시, 어형은 학생이 바꾸게):\n"
        "  · given_low(하): 정답 문장의 '거의 모든' 핵심 단어를 제공(배열에 가까운 쉬움).\n"
        "  · given_mid(중): 핵심 단어의 '절반 정도'만 제공.\n"
        "  · given_high(상): '최소한의 1~2개'만 제공(또는 빈 배열로 아무것도 안 줌).\n"
        "  세 목록은 given_low ⊇ given_mid ⊇ given_high 처럼 개수가 하>중>상 순으로 줄어들게 하고, "
        "모두 정답 문장에 실제로 쓰인 단어여야 합니다. 기능어(관사·전치사)는 굳이 안 넣어도 됩니다.\n"
