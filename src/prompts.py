...
def exam_prompt(title: str, body: str, grammar: schemas.GrammarSection | None,
    ...
    return (
        ...
        "[함축의미] 그대로 직역하면 뻔한 표현은 제외하고, 지문의 '내용을 알아야만' 이해되는 구문·표현만 "
        "골라 각 줄에 다음 형식으로 한글로 풀어 쓰세요:\n"
        "   '{문장번호}문장의 \"{원문 표현}\"은 문맥상 ({한글로 푼 의미})을 의미함'\n\n"
        "[서술형] 영작 문제로 나올 만한 '문법적으로 어려운 문장'을 골라 각 줄에 다음 형식으로 쓰세요:\n"
        "   '{문장번호}문장 → {영작 시 어려운 이유}'\n"
        "   선정 기준: 5형식, 분사구문, 관계부사, 강조구문, 가정법, 비교구문 등 어려운 문법이 쓰인 문장.\n"
        + ref + "\n\n"
        + _passage_block(title, body)
    )
