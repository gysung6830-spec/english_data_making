...
- 그 '문장 문맥'에 넣었을 때 '뜻이 자연스럽게 통하는(=정답이 될 수 있는)' 보기가 몇 개인지 세어 valid_count 에 넣는다.
  ...
  · ★ '뜻이 통함'만 보지 말고, 그 자리에 넣었을 때 '문장이 문법적으로도 자연스러운지'(동사 패턴·전치사·
    연어까지)를 반드시 함께 본다. 뜻은 비슷해도 연어·문법이 어긋나면 정답이 아니다.
    예: "it reminded him of a fire" 에서 remind 는 'remind A of B' 로 맞지만, recall 은 'recall A of B'
       형태로 쓰지 않는다 → recalled 는 문법 불일치이므로 정답이 아니다(→ 이 문항의 valid_count 는 1).
    예: 자동사/타동사, of·about·with 등 전치사 결합, 가산/불가산 등이 어긋나는 보기도 정답에서 제외한다.
- ok 판정:
