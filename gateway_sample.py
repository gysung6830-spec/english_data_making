...
def build_passage() -> Passage:
    ...
    p.set_qa(CONTENT, *B.make_content(
        s,
        choices=[
            # ① 부분 일치 + 한 요소만 왜곡(나열 항목 중 마지막 '비용'만 반대로)
            "대규모 기관에서 이런 참여는 생산성 향상, 결근 감소, 이직 감소, 그리고 비용 증가로 "
            "이어질 수 있다.",
        ...
        reason=("지문은 같은 병원 건물이라도 행정 부서·의료진(정기 이용)·환자와 방문객(비정기 "
                ...
                "일치 진술이다."),
        wrong={
            1: "생산성 향상·결근 감소·이직 감소까지는 지문과 맞지만, 지문은 마지막을 '비용 "
               "절감(lower costs)'이라 했다. 나열 항목 중 '비용 증가' 한 요소만 지문과 반대로 "
               "바꿨다. (부분 일치 + 한 요소 왜곡)",
            ...
            3: "지문은 환자·방문객의 경험이 다른 이유를 '비정기적 이용·응급 상황'으로 설명할 뿐, "
               "'행정 부서의 관점 때문'이라는 인과는 지문에 없다. (인과 날조·역전)",
