...
lineup-0-all.png                  아홉 종 전체 목차 (실물 사진 없이 번호·이름·한 줄)
lineup-1-read.png                 01-03 읽는 자료 — 목록 + 실물 + 차별화 포인트
lineup-2-pilsaengbo.png           04-05 시그니처 자료
lineup-3-write.png                06-09 쓰는 자료
lineup-books.png                  교재
...
**라인업은 네 장**입니다. `lineup-0-all.png` 은 처음 온 사람에게 보여 주는
전체 목차라 실물 사진 없이 번호·이름·한 줄만 세웁니다. 나머지 세 장은
묶음별 소개라 실물 썸네일과 `이 셋이 다른 점` 이 함께 붙습니다.
