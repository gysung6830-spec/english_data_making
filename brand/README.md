# Ortica 블로그 브랜드 키트

네이버 블로그에 바로 올릴 이미지 43종과, 자료별 상세페이지 원고 초안 10편.
라인업·상세페이지에 들어가는 자료 예시는 **이 저장소의 생성기가 실제로 뽑은
출력물**입니다. 그림으로 흉내 낸 게 아닙니다.

```
brand/
├─ catalog.py       자료·교재 문구, 가격, 예시 이미지 연결 ← 고칠 곳은 대부분 여기
├─ ortica_brand.py  팔레트 · 잎 마크
├─ build.py         레이아웃 정의 + CLI
├─ sample_shots.py  저장소 생성기로 예시 이미지 뽑기 (구 버전 자료용)
├─ pdf_samples.py   실제 판매 PDF 에서 예시 이미지 뽑기 ← 지금 쓰는 쪽
├─ ANALYSIS.md      자료 분석과 판매 차별화 포인트
├─ render.py        HTML → PNG (헤드리스 크로미움)
├─ assets/          결과 이미지 (samples/ 는 실제 산출물, thumbs/ 는 목록용 썸네일)
├─ posts/           블로그 원고 초안 10편
├─ PRICING.md       가격 근거와 확인해야 할 것
└─ VOICE.md         누가 읽는가 + "AI가 쓴 것 같다"는 말을 안 듣기 위한 문체 규칙
```

---

## 1. 브랜드

| 문구 | 값 |
| --- | --- |
| 이름 | Ortica · 오르티카 영어 |
| 콘셉트 | 읽고, 뜯고, 시험에 나오는 문제만 풉니다 |
| 소개 | 고등영어자료 · 모의고사 · 내신 · 수능자료 제작 (글줄 없이 칩으로만) |

*Ortica* 는 이탈리아어로 쐐기풀입니다. 로고는 톱니가 살아 있는 잎 한 장.

| 이름 | 값 | 쓰임 |
| --- | --- | --- |
| Green 900 | `#10382C` | 어두운 배경 |
| Green 700 | `#1B5A46` | 밝은 배경 위 글자 |
| Leaf | `#8ACB5E` | 잎 · 강조 |
| Cream / Paper | `#F5F0E2` / `#FFFDF7` | 밝은 배경 |
| Gold | `#DCA945` | 아주 짧은 강조만 |
| Muted | `#7E9086` | 보조 설명 |

글꼴은 저장소에 이미 있는 **나눔스퀘어라운드**(`templates/fonts/`). 분석지 PDF와
같은 글꼴이라 블로그와 자료의 인상이 이어집니다.

## 2. 어디에 무엇을 올리나

### 네이버 블로그

**프로필** — 네이버에는 **`profile-mark-800.png`** 를 올리세요.

| 파일 | 크기 | 쓰는 곳 |
| --- | --- | --- |
| `profile-mark-800.png` | 800×800 | **네이버 권장.** 심볼만 |
| `profile-mark-400.png` | 400×400 | 같은 판, 작은 용량 |
| `profile-mark-light-800.png` | 800×800 | 밝은 배경, 심볼만 |
| `profile-light-1200.png` | 1200×1200 | 크림 바탕 + Ortica + 오르티카 영어 |
| `profile-light-800.png` | 800×800 | 위와 같음 |
| `profile-light-400.png` | 400×400 | 위와 같음 |
| `profile-name-800.png` | 800×800 | 크게 보이는 자리용(이름 포함) |
| `profile-name-400.png` | 400×400 | 위와 같음 |
| `profile-portrait-400x480.png` | 400×480 | 사이드바가 세로로 긴 스킨 |

**왜 심볼만인가.** 네이버 앱은 프로필을 지름 60px 남짓한 원으로 줄입니다.
그 크기에서 'Ortica 영어 / 오르티카 영어' 두 줄은 반드시 뭉개집니다. 글자를
빼고 잎을 키우면 44px 까지 줄어도 형태가 남습니다.

**올릴 때는 800px 판을 올리세요.** 161px 짜리를 올리면 네이버가 그걸 다시
늘렸다 줄이면서 더 흐려집니다. 큰 원본을 주고 축소를 네이버에 맡기는 편이
선명합니다.

**웹(PC)용 블로그 타이틀** — 가로 **966px 고정**, 세로 50~600px

| 파일 | 크기 | 성격 |
| --- | --- | --- |
| `title-966x550-dark.png` | 966×550 | 배너형. 브랜드가 먼저 읽힌다 |
| `title-966x550-light.png` | 966×550 | 배너형(밝은 배경) |
| `title-966x420-dark.png` | 966×420 | 배너형(조금 낮게) |
| `title-966x300-dark.png` | 966×300 | 가로형. 글 목록이 위로 올라온다 |
| `title-966x300-light.png` | 966×300 | 가로형(밝은 배경) |
| `title-966x200-dark.png` | 966×200 | 슬림 |

배너형(550)은 벤치마킹하는 블로그와 비슷한 비율입니다. 타이틀을 크게 쓰면
브랜드가 먼저 읽히고, 낮게 쓰면 글 목록이 위로 올라옵니다. 소개 칩은 콘셉트
아래에 놓입니다.

관리 → 꾸미기 설정 → 타이틀 → 직접등록. **'블로그 제목 표시'는 꺼 주세요.**
이미지 안에 제목이 이미 들어 있어서 켜면 글자가 겹칩니다.

**모바일 홈 커버** — 블로그앱 → 홈편집 → 커버 이미지

| 파일 | 크기 |
| --- | --- |
| `cover-backdrop-1600x1200.png` | 1600×1200 (기본) |
| `cover-backdrop-2400x1350.png` | 2400×1350 (가로가 넓은 기기) |
| `cover-backdrop-1080x1080.png` | 1080×1080 (정사각) |

**여기엔 글자가 없습니다. 일부러 뺐습니다.** 네이버가 이 이미지 **위에**
블로그 제목·프로필·이웃수·홈편집 버튼을 직접 그립니다. 이미지에 브랜드명을
넣으면 그 위에 네이버가 또 제목을 얹어서 글자가 겹칩니다.

기기마다 잘리는 자리도 다릅니다(아이패드는 가운데 가로 띠만 보입니다). 그래서
한 곳에 그림을 몰지 않고 잎을 흩어 놓았고, 아래쪽은 흰 글자가 얹히는 자리라
한 겹 어둡게 눌러 뒀습니다.

블로그 제목은 이미지가 아니라 **블로그 설정의 '블로그명'** 에 넣으세요.
지금 설정하신 "필자의 생각이 보이는 영어: Ortica 영어" 처럼요.

글자가 들어간 판은 같은 규격으로 다 있습니다 — `cover-branded-1600x1200.png` ·
`-2400x1350` · `-1080x1080` · `-1200x900`. 위에 아무것도 안 얹히는 자리(공유
이미지·배너)에 쓰시고, 홈 커버로 쓰시려면 네이버 제목과 겹칠 수 있다는 것을
감안하세요.

### 그 밖

| 용도 | 파일 |
| --- | --- |
| 공유 미리보기(OG) | `thumb-og-1200x630-sample.png` |
| 파비콘 · 앱 아이콘 | `favicon-32/180/512.png` |
| 가로 로고(배경 있음) | `logo-horizontal-solid-light.png` / `-solid-dark.png` |
| 가로 로고(배경 투명) | `logo-horizontal-on-light.png` / `-on-dark.png` |

**투명 PNG 주의.** `-on-light` / `-on-dark` 는 배경이 없는 판이라 다른 이미지
위에 얹을 때만 씁니다. 그냥 올리면 보는 쪽 배경색에 따라 글자가 안 보입니다
(`-on-dark` 는 밝은 글자라 흰 배경에서 안 보이고, `-on-light` 는 그 반대).
어디에 올릴지 모르겠으면 **`-solid-` 쪽을 쓰세요.**

## 3. 라인업과 상세페이지

```
detail-03-pilsaengbo-class.png    자료 한 종 = 상세페이지 한 장
lineup.png                        라인업 전체 한 장
lineup-1-read.png                 └ 나눈 조각 — 머리말 + 01-02
lineup-2-pilsaengbo.png             03-04
lineup-3-write.png                  05-08 + 꼬리말
posts/03-pilsaengbo-class.md      본문 원고 초안
```

**자료 하나에 이미지 하나.** 예전에는 한 자료를 표지·구성·안내 세 장으로
쪼갰는데, 올릴 때마다 순서를 맞춰야 하고 독자는 같은 이야기를 세 번
스크롤합니다. 지금은 한 장에 세로로 다 세웁니다.

```
번호 · 이름 · 영문
한 줄
차별점 한 문장
구성 표 (회차·난이도 같은 것)
(반 페이지 — opening 을 넣은 자료만)
(특징 묶음 제목 — points_title 을 넣은 자료만)
포인트 → 사진, 포인트 → 사진 … (넷, 자료에 따라 다섯·여섯)
이런 분께
가격 (catalog.py 에 넣으면 나옵니다)
```

**상세페이지·라인업은 3배 크기로 저장합니다**(`build.py` 의 `DOC_SCALE = 3`).
900px 판이 2700px 파일로 나옵니다. 네이버가 올린 이미지를 본문 폭에 맞춰
줄여 주므로, 큰 판을 주고 축소를 맡기는 편이 글자가 선명합니다. 프로필·타이틀
(966px 고정)·파비콘처럼 규격이 정해진 이미지는 그대로 1배입니다.

**포인트 하나에 사진 하나.** 지면을 위쪽에 몰아 놓고 아래에서 설명하면
독자가 위아래를 오갑니다. 포인트를 한 줄로 짚고 바로 그 자리를 보여 주는
리듬으로 갑니다.

**지면은 통째로 넣지 않고 그 포인트가 보이는 자리만 잘라 붙입니다.** A4 한 장을
900px 폭에 밀어 넣으면 글자가 3mm 가 되어 아무것도 안 읽힙니다. "함축의미를
카드로 따로 뽑았습니다" 아래에는 그 카드만, "어법은 동그라미를 치면서" 아래에는
그 태그 줄만 놓습니다.

- 자를 자리는 `pdf_samples.py` 의 `CROPS` 에 `(원본, 저장 이름, (x0,y0,x1,y1))`
  로 적습니다. 좌표는 0~1 비율이라 dpi 를 바꿔도 그대로 씁니다.
- `python brand/pdf_samples.py --crops-only` 로 PDF 없이 조각만 다시 자릅니다.
- 특징에 조각을 달려면 `catalog.py` 의 `points` 를 세 칸으로 씁니다 —
  `(소제목, 설명, "c-analysis-point.png")`. `|` 로 나누면 두 장을 잇달아 놓습니다.
- 조각은 **원래 지면에서 차지하던 비율대로** 보여 줍니다(`CROP_ZOOM = 1.5`).
  좁은 상자를 칸 폭까지 늘리면 확대되어 흐려지고, 지면에서 작은 상자였다는
  사실도 사라집니다. 반대로 너무 작아지지 않게 `CROP_FLOOR = 0.62` 로 아래를
  막아 뒀습니다. 어느 쪽이든 원본보다 크게 늘리지는 않습니다.
- 한 특징에 딸린 조각들의 세로 합은 `POINT_STACK_MAX = 300` 을 넘지 않습니다.
  설명 서너 줄이 차지하는 높이에 맞춘 값입니다. 더 길어지면 오른쪽 설명이 진작
  끝나 옆이 텅 비므로, 묶음째 같은 비율로 줄입니다. 이 값을 키우면 사진이
  커지는 대신 옆에 여백이 생기고 장도 길어집니다.
- 조각은 예외 없이 **왼쪽 칸, 설명은 오른쪽**입니다. 납작한 띠도 마찬가지로
  옆칸에 넣습니다(칸 폭을 넘지 않으니 실제로는 칸을 꽉 채웁니다).
- 같은 자리를 방식만 바꿔 낸 문항들은 `pdf_samples.py` 의 `LABELED` 에 적으면
  라벨을 붙여 세로로 이어 붙인 한 장이 나옵니다(어휘 세 종 비교).
- **사진 없는 특징은 상세페이지에 싣지 않습니다.** 말로만 하는 줄이 하나 끼면
  그 옆의 실물 사진까지 '광고 문구'로 읽힙니다. `build.py` 가 조각이 없거나
  파일이 사라진 특징을 자동으로 빼고, 뺄 때마다 무엇을 뺐는지 찍어 줍니다.
  자료 전체에 조각이 하나도 없으면(아직 PDF 를 못 받은 자료) 글로만 세웁니다.
- 조각이 붙은 자료는 지면 전체 사진을 넣지 않습니다.
- **유형이 여럿인 자료는 `gallery` 로 전부 펼쳐 놓습니다.** 이름만 늘어놓으면
  "그래서 그게 어떻게 생겼는데"가 남습니다. `catalog.py` 의
  `gallery=[(파일명, 라벨)]` 과 `gallery_title` 을 채우면 특징 아래에 두 칸으로
  흘려 놓습니다. 지금은 쓰는 자료가 없습니다 — 17종 변형문제에 한 번 붙였다가,
  같은 지문이 열일곱 번 나와 장이 너무 길어져 뺐습니다.
  조각은 `python brand/pdf_samples.py --src <폴더> --gallery-only` 가 만듭니다 —
  학생용 열일곱 쪽에서 유형칸을 한 장씩 따고, 열일곱 쪽에 똑같이 실린 지문은
  건너뛰어 지시문 위쪽과 지문 아래쪽만 이어 붙입니다. 어법·어휘처럼 지문 안에
  밑줄로 묻는 유형은 지문이 곧 문제라서 통째로 남깁니다.

**형태·구성·배포(`spec`)는 이미지에 넣지 않습니다.** `PDF`, `구매 후 즉시
다운로드` 처럼 자료마다 똑같은 줄이 장마다 반복되면 아래쪽이 안 읽힙니다.
`catalog.py` 의 `spec` 은 그대로 두어 원고 초안(`posts/*.md`)에는 계속
들어가니, 판매 글 본문에서 쓰시면 됩니다.

**라인업은 한 흐름으로 짜고 묶음 경계에서만 자릅니다.** 묶음마다 디자인을
새로 시작하면 어디까지가 한 목록인지 안 보입니다. 그래서 띠를 쌓아 한 장으로
짠 다음, 그 띠 경계에서 넷으로 나눕니다. 순서대로 올리면 이어진 한 장처럼
읽히고, 한 장으로 올리고 싶으면 `lineup.png` 하나만 쓰면 됩니다.

```
머리말 (어두움)          읽고 · 뜯어보고 · 손으로 씁니다  ┐ lineup-1
01 — 02 읽는 자료 (밝음)   썸네일 + 한 줄 + 특징 2줄      ┘
03 — 04 시그니처 (어두움)  필생보                          → lineup-2
05 — 08 쓰는 자료 (밝음) + 꼬리말                          → lineup-3
```

바탕색이 밝은 판과 어두운 판으로 번갈아 가므로 어디서 묶음이 바뀌는지 눈에
보입니다. 조각 오른쪽 위의 `2 / 3` 은 올리는 순서가 흐트러지지 않게 하려는
표시라, 이어 붙인 `lineup.png` 에는 나오지 않습니다.

묶음 구성은 `build.py` 의 `lineup_sections()` 하나만 고치면 됩니다. 조각 파일
이름은 그 아래 `LINEUP_PARTS` 입니다.

**제작 교재(고3)는 라인업에서 뺐습니다.** 실물 PDF 가 없어 예시 칸이 빈
점선으로 남았습니다. `catalog.py` 의 `BOOKS` 와 상세페이지
(`detail-b1-voca.png`)는 그대로 두었으니, PDF 를 받으면 `lineup_sections()`
에 한 줄만 다시 넣으면 됩니다.

**묶음별 `이 셋이 다른 점` 세 칸도 뺐습니다.** 자료마다 차별점 한 문장
(`catalog.py` 의 `edge`)이 상세페이지에 이미 굵게 들어가 있어서, 라인업에서
같은 이야기를 한 번 더 하면 목록이 늘어지기만 합니다.

**01 지문자료에는 차별점 한 줄이 없습니다.** 기본 자료라 팔 것이 판형 선택뿐입니다.
`edge` 를 비워 두면 상세페이지에서 그 강조 상자가 통째로 빠집니다.

**주문제작자료**는 `catalog.py` 의 `made_to_order=True` 로 표시합니다. 받아
놓고 파는 자료가 아니라 주문을 받아 만드는 자료라는 뜻이고(필생보 독학용,
동형모의고사), 라인업과 상세페이지에 테두리 칩으로 붙습니다. 배포 방식이
다르니 눌러서 바로 받는 자료와 섞이면 안 됩니다.

**시그니처 자료**는 `catalog.py` 의 `signature=True` 로 정합니다. 지금은
필생보·독학용과 17종 변형문제, 동형모의고사 넷입니다. 라인업에서는 이름 옆에
금색 `SIGNATURE` 칩이 붙고, 상세페이지는 혼자 어두운 판이 되며 배지와 함께
이름이 더 크게 나옵니다.

분량은 상세페이지끼리 맞춥니다. `build.py` 위쪽의 두 상수가 그 기준입니다.

- `MAX_FIGURES = 2` — 실물 사진은 두 장까지. 더 넣으면 스크롤만 길어지고
  끝까지 안 읽힙니다. 나머지 지면은 `catalog.py` 의 `extra` 에 남아 있으니
  순서만 바꾸면 다른 장을 앞세울 수 있습니다. 자료마다 다르게 하려면
  `catalog.py` 의 `figures` 로 덮어씁니다(01 지문자료는 판형 셋을 다 보여야
  해서 `figures=3`).
- `MAX_POINTS = 4` — 특징은 네 줄까지. 자료마다 다르게 하려면 `catalog.py`
  의 `max_points` 로 덮어씁니다(필생보는 여덟, 통합 워크북은 다섯).
- 라인업 줄의 부제는 한 줄(`one_line`)입니다. 목록에서 더 중요한 말이 따로
  있으면 `lineup_line` 으로 갈아 끼웁니다(17종 변형문제). 한 줄이 비어 있으면
  차별점(`edge`)이 대신 들어갑니다(통합 워크북).
- 라인업 줄에는 특징을 **둘**만 보여 줍니다. `lineup_points` 로 더 많이
  보이게 하면 설명을 **첫 문장까지만** 잘라 붙입니다.
- 설계가 유형마다 다른 자료는 상세 문구를 잘라 붙여서는 두 줄밖에 못 담아
  '순서 배열만 특이한 자료'로 읽힙니다. 그럴 때는 `lineup_bullets` 에 목록 전용
  한 줄짜리를 따로 적습니다(17종 변형문제는 여덟). 이미 줄여 쓴 것이라 첫 문장
  자르기는 걸리지 않습니다.
  생김새는 그대로 둡니다 — 그 자료만 다른 모양이면 목록에서 튑니다.

특징 설명도 카드마다 250~330자 안에 들어오게 씁니다. 한 카드만 길면 목록으로
늘어놨을 때 그 카드만 눈에 걸립니다.

**문구는 사는 사람 쪽에서 씁니다.** 읽는 사람은 학원·과외 선생님과 혼자 공부하는
학생이지 만든 사람이 아닙니다. "평가원식 5축으로 설계했습니다"가 아니라 "찍어서는
못 맞힙니다" 쪽입니다. 규칙은 [VOICE.md](VOICE.md) §0.

### 실제 자료 예시

라인업과 상세페이지에 들어가는 그림은 **실제 판매 PDF 를 찍은 것**입니다.

```bash
pip install pymupdf
python brand/pdf_samples.py --src ~/Downloads/ortica-pdf
```

`pdf_samples.py` 의 `SHOTS` 에 (파일명 조각, 페이지, 저장 이름, 남길 비율)을
적어 두면 그 페이지를 PNG 로 뽑습니다. 파일명이 바뀌어도 조각으로 찾습니다.

`SHOTS` 는 **파일 이름이 아니라 첫 쪽 머리말의 문구**로 자료를 가려냅니다.
내려받을 때마다 파일명이 바뀌고 한글이 깨져 들어오기 때문입니다.

지면 전체는 아래처럼 넉넉히 뽑아 둡니다. 상세페이지에 실제로 붙는 것은 여기서
잘라 낸 **조각**(`c-*.png`, `CROPS`)이고, 지면 전체는 라인업 썸네일과 판형
비교에 씁니다.

| 자료 | 뽑아 둔 장수 | 파일 |
| --- | --- | --- |
| 지문자료 | 3 | `passage.png` · `one-line.png` · `one-line-2col.png` (판형 3옵션) |
| 지문분석지 | 6 | `analysis.png` · `-point` · `-flow` · `-blank` · `-vocatest` · `-guide` |
| 필생보 | 7 | `-guide` · `pilsaengbo`(통독·어휘) · `-sent` · `-summary` · `-grammar` · `-student` · `-student2` |
| 필생보 독학용 | 6 | `psb-solve.png` · `-answer` · `-predict` · `-paraphrase` · `-signal` · `-pace` |
| 통합 워크북 | 2 | `workbook-integrated.png` · `-en` |
| 서술형 대비 교재 | 1 | `workbook.png` |
| 17종 변형문제 | 7 | `variation.png` · `-gram` · `-gramwrite` · `-oxen` · `-teacher` · `-answer` · `-quick` |
| 동형모의고사 | 5 | `mock.png` · `-paper` · `-teacher` · `-answer` · `-quick` |

**표지와 목차는 넣지 않습니다.** 사는 사람이 궁금한 건 겉이 아니라 안입니다.
실제로 푸는 지면, 해설 지면, 학생용 빈칸본처럼 **내용이 든 쪽**만 고릅니다.

`PAIRS` 에 적어 두면 같은 쪽의 두 판본을 나란히 붙인 그림도 만듭니다.
'강사용은 채워져 있고 학생용은 비어 있다' 는 글로 설명하기보다 나란히 놓는 편이
빠릅니다.

**평가원 VOCA** 는 아직 실물을 못 봐서 자리를 비워 뒀습니다
(목록에 '예시 준비 중'으로 표시). PDF 를 주시면 `SHOTS` 에 한 줄 넣고 다시
돌리면 들어갑니다.

**지문 저작권.** 예시 이미지에 EBS·평가원 지문이 들어갑니다. 한 지문 전문이
그대로 다 보이지 않도록 잘라 쓰는 편이 안전합니다. 자세한 건
[ANALYSIS.md](ANALYSIS.md) §6.

### 자료 분석

[ANALYSIS.md](ANALYSIS.md) 에 자료별 구성과 **판매 차별화 포인트**를
정리했습니다. 라인업 소개글과 상세 판매 포스팅은 이 문서를 재료로 씁니다.

### 라인업을 세 묶음으로 나눈 이유

여덟 개를 한 줄로 늘어놓으면 목록으로만 읽힙니다. 묶어 두면 흐름이 생깁니다.

- **01–02 읽는 자료** — 지문자료 · 지문분석지
- **03–04 시그니처 자료** — 필생보 · 독학용
- **05–08 쓰는 자료** — 통합 워크북 · 서술형 · 변형문제 · 동형모의고사

읽고 → 뜯어보고 → 손으로 쓰는 순서입니다. 문구는 `build.py` 의
`lineup_sections()` 에서 고칩니다.

### 세로 길이는 브라우저가 잽니다

라인업·상세페이지는 내용에 따라 길이가 달라집니다. 글자 수로 어림하면 한글
줄바꿈과 이미지 비율 때문에 늘 어긋나서, 본문 끝에 표식 줄을 넣고 한 번 그려
그 위치를 읽은 뒤 그 높이로 다시 그립니다(`render.measure_height`).

### 전부 한눈에 보기

```bash
python brand/build.py index      # brand/assets/_index.png
```

결과물을 파일명·크기와 함께 한 장에 늘어놓습니다. 투명 PNG 는 흰 바탕 위에
얹혀 보이므로, 여기서 비어 보이는 파일이 곧 "배경이 투명한 파일"입니다.

## 4. 고치고 다시 뽑기

```bash
pip install pillow pydantic jinja2 pyyaml   # 최초 1회
python brand/sample_shots.py                # 실제 자료 예시
python brand/build.py all                   # 이미지 + 원고 전부
```

| 고칠 것 | 고칠 파일 |
| --- | --- |
| 자료 이름·설명·구성·대상·가격·예시 연결 | `catalog.py` |
| 브랜드명·콘셉트·소개 칩 | `build.py` 위쪽 `BRAND_KO`, `CONCEPT`, `KEYWORDS` |
| 색 | `ortica_brand.py` 의 `PALETTE` |
| 잎 모양(톱니 개수·깊이·비율) | `ortica_brand.py` 의 `leaf_path`, `logomark_svg` |

자료 하나만:

```bash
python brand/build.py item analysis
python brand/build.py posts          # 원고 초안만
```

포스트 썸네일:

```bash
python brand/build.py thumb \
  --title "빈칸추론이 안 풀리는<br>진짜 이유 세 가지" \
  --sub "지문 구조부터 다시 보는 독해법" \
  --tag "수능 독해" --number "No.01" \
  --out brand/assets/post-001.png
```

`--size 1200x630` 으로 공유용을, `--light` 로 밝은 배경을 뽑습니다. 제목은
`<br>` 로 직접 끊는 편이 보기 좋습니다.

## 5. 다른 PC 에서

렌더링은 헤드리스 크로미움을 그대로 부릅니다. 크롬을 못 찾으면 경로를 알려
주세요.

```bash
CHROME_BIN="/Applications/Google Chrome.app/Contents/MacOS/Google Chrome" \
  python brand/build.py all
```
