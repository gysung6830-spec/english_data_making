...
**상세페이지·라인업은 2배 크기로 저장합니다**(`build.py` 의 `DOC_SCALE = 2`).
900px 판이 1800px 파일로 나옵니다. 네이버가 올린 이미지를 본문 폭에 맞춰
...
- 한 특징에 딸린 조각들의 세로 합은 `POINT_STACK_MAX = 620` 을 넘지 않습니다.
  더 길어지면 오른쪽 설명이 진작 끝나 옆이 텅 비므로, 묶음째 같은 비율로
  줄입니다. 그래도 비면 조각 수를 줄이는 편이 낫습니다.
