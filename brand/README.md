...
lineup.png                        자료 전체 라인업 = 이미지 한 장 (900×5285)
...
**라인업도 한 장**입니다. 묶음마다 이미지를 나눠 두면 블로그에 올릴 때 순서가
흐트러지고, 읽는 사람은 어디까지가 한 목록인지 모른 채 끊깁니다. 한 장에 다
세우고 묶음은 **바탕색으로** 가릅니다 — 밝은 판과 어두운 판이 번갈아 나오면
어디서 묶음이 바뀌는지 눈에 보입니다.

```
머리말 (어두움)          읽고 · 뜯어보고 · 손으로 씁니다
01 — 03 읽는 자료 (밝음)   실물 썸네일 + 한 줄 + 특징 2줄 · 이 셋이 다른 점
04 — 05 시그니처 (어두움)  필생보
06 — 09 쓰는 자료 (밝음)
교재 (어두움)             B1
```

묶음 구성은 `build.py` 의 `lineup_sections()` 하나만 고치면 됩니다.
