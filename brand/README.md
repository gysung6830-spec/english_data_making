...
한 줄
차별점 한 문장
반 페이지 — 지면이 어떻게 생겼는지 한 번
구성 표 (회차·난이도 같은 것)
특징 넷 — 한 줄 설명 + 그 특징이 보이는 지면 조각
이런 분께
가격 (catalog.py 에 넣으면 나옵니다)
```

**지면을 한 번은 크게, 그 다음은 조각으로.** 위쪽에 `half` 로 **반 페이지**를
놓아 무엇을 파는지 지면으로 보여 주고, 아래에서 특징을 하나씩 뜯습니다.
조각만 늘어놓으면 부품은 알아도 지면이 어떻게 생겼는지가 안 그려집니다.

**나머지 지면은 통째로 넣지 않고 특징이 보이는 자리만 잘라 붙입니다.** A4 한 장을
