...
카드는 **한 줄 → 강조 → 실물 사진 전부 → 특징 전부** 순서로, 그 자료 하나로
완결되는 상세페이지입니다. 세로가 3,000px 을 넘기도 하는데 네이버 본문은
세로로 길어도 문제가 없습니다. 대신 **문장은 짧게** 씁니다. 길이는 실물 사진이
차지하고, 글은 훑어 읽히게 두는 편이 낫습니다.

**필생보는 시그니처 자료**라 카드가 다릅니다. 혼자만 어두운 판에
`SIGNATURE · 시그니처 자료` 배지를 달고, 이름을 더 크게, 특징을 여섯 줄까지
