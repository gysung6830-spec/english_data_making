...
CONCEPT = "읽고, 뜯고, 시험에 나오는 문제만 풉니다"
# 소개 문구는 글줄로 반복하지 않고 칩으로만 보여 준다.
KEYWORDS = ["시험에 적합한 고등영어자료", "모의고사 · 내신", "수능자료 제작"]
...
def lineup_rows(items, t: dict[str, str], u: float, thumb_w: int = 320,
    ...
    for it in items:
        ...
        head_el = f"""<div style="display:flex;align-items:baseline;flex-wrap:wrap;
            ...
            {f'<div class="sans" style="margin-top:{u * .9:.0f}px;font-size:{u * 1.6:.0f}px;color:{t["accent"]};line-height:1.6;word-break:keep-all">{it.lineup_note}</div>' if it.lineup_note else ''}
