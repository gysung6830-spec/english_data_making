...
---
## ★ 이 배치(전기·설명문) 추가 지침 — Frances Glessner Lee
- 이 지문은 '과학 수사의 어머니' Frances Glessner Lee의 **전기(설명문)**다. 시간순 생애 + 업적 설명.
...
- **소단원은 크게 묶여 있다(6~11문장).** 각 소단원에 flow_blocks 3~5개로 큰 흐름을 나눠 요약하라.
- **misreads(오답)**: 어휘 오역이 아니라 '**사건·생애 사실(인과·시점) / 인물의 의도·신념 / 함축(반어·속뜻) / 지칭(This·It·they·she 등이 무엇을 가리키나)**'를 겨냥. 예: 하버드가 여성을 안 받아 결혼할 수밖에 없었던 인과, 미니어처를 만든 목적(형사 훈련), 'This process'=앞의 증거수집·분석 과정(지칭), "mother of forensic science"는 그녀 기여에 대한 칭호(함축). 각 소단원에 지칭·함축 최소 한 번씩.
- stance: 생애·업적을 긍정적으로 조명 → "긍정적" 또는 "중립적". structure: 시간순 전기 → "시간·순서(나열)"가 자연스럽고, 도입부(과학수사 설명)는 "주장→근거·예시"도 가능.
