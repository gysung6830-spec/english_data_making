...
.card{border:1px solid var(--line); border-radius:9px; padding:7px 11px 8px; margin-bottom:8px; break-inside:avoid;}
...
.ox{padding:5px 0 5px; border-bottom:1px dotted #e6ebe8; break-inside:avoid;}
.ox:last-child{border-bottom:none;}
.st{font-size:9.4pt; line-height:1.5;}
.st .sid{display:inline-block; min-width:26px; color:var(--sub); font-size:7.6pt; font-weight:700;}
.st .en{color:#2b2f6b; font-style:italic;}
.pick{margin:3px 0 0 26px; font-size:8.8pt; color:var(--sub);}
...
.why{margin:3px 0 0 26px; font-size:8.6pt; color:#3a4250; line-height:1.5;}
    ...
    for s in p["sentences"]:
        for m in s.get("misreads",[]):
            ...
            if ans:
                ...
                h.append('<div class="ox">')
                h.append(f'<div class="st">{sid}{stmt}</div>')
