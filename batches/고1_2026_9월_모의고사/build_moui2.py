...
if os.environ.get("RENDER") and ok:
    ...
    render_vocab_test(P, SC+"/고1_모의고사_단어시험", source_title=SRC, footer_note=FOOT)
    for suf in ["학생용","강사용"]:
        dd=fitz.open(f"{stem}_{suf}.pdf"); print(f"  {suf}: {dd.page_count}p"); dd.close()
    for suf in ["학생용","정답"]:
        dd=fitz.open(f"{SC}/고1_모의고사_단어시험_{suf}.pdf"); print(f"  단어시험 {suf}: {dd.page_count}p"); dd.close()
    print("RENDER OK")
