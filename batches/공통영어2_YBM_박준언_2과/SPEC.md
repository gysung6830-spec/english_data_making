...
---
## ★ 이 배치(내러티브) 추가 지침 — 소설 「Dry」
- 이 지문은 **설명문이 아니라 1인칭 서사(소설)**다. 문장(beat)에는 대화·묘사·짧은 조각이 섞여 있다.
- **key_grammar 는 배정 JSON의 `key_grammar_hint` 로 지정된 어법을 그대로 사용**하라(소단원마다 서로 다르게 배정해 두었으니 합본 중복이 없다). source_sentence 는 그 어법이 실제로 들어간 이 소단원 beat 원문으로.
- **misreads(오답)**: 어휘 오역이 아니라 '**줄거리 사실 / 인물의 의도·감정 / 반어·복선(함축) / 지칭(누가·무엇을 가리키나)**'를 겨냥하라. 예: 정장 남자의 '도움'이 사실은 얼음을 가로채려는 것(함축), 'This/it/she/he'가 누구·무엇인지(지칭). 각 소단원에 지칭·함축 오답이 최소 한 번씩.
- theme_ko·topic·flow_blocks 는 '그 장면에서 무슨 일이 일어나는가'를 서사적으로 요약.
- stance: 서사 묘사는 보통 "중립적"(작가가 상황을 보여줌). 결말/경고 부분(작품 정리·기아석)은 "부정적·비판적"(경고) 가능.
- structure: 장면 전개는 "시간·순서(나열)"가 자연스럽다. 경고성 결말은 "문제→해결(방안)"/"예시→일반화(결론)".
