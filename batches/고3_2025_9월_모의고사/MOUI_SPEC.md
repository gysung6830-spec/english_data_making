...
입력: /.../scratchpad/moui_data.json (14지문. 각 item_no·theme·sentences[id,english,korean(공식 한줄해석 참고용)])
출력: 배정된 지문마다 /.../scratchpad/moui/passages/p<item>.json  (한 지문 = 한 파일)
※ english 원문은 moui_data.json 그대로 사용(수정 금지). korean 은 참고용(직접 새로 써도 됨).
