...
import fitz
SC="/tmp/claude-0/-home-user-english-data-making/3e2ff8b7-89bb-5341-95ca-4062ce95757b/scratchpad/moui4"
SRC="고3 2026년 9월 모의고사"   # 출처 라벨
...
if os.environ.get("RENDER") and ok:
    ...
    render_vocab_test(P, SC+"/고3_모의고사_단어시험", source_title=SRC, footer_note=FOOT)
    for suf in ["학생용","강사용"]:
        dd=fitz.open(f"{stem}_{suf}.pdf"); print(f"  {suf}: {dd.page_count}p"); dd.close()
    for suf in ["학생용","정답"]:
        dd=fitz.open(f"{SC}/고3_모의고사_단어시험_{suf}.pdf"); print(f"  단어시험 {suf}: {dd.page_count}p"); dd.close()
    print("RENDER OK")
