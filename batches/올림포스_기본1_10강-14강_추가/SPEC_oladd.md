...
---
## ★ 이 작업(올림포스 합본 추가) 추가 지침
- 이 지문들은 **EBS 올림포스 영어독해 기본1(수능형 독해)**로, 기존 '올림포스 10강-14강 합본'에 **추가**되는 소단원이다.
- 배정 JSON의 각 소단원에 `key_grammar_hint`(사용할 핵심문법)와 `ko_gloss`(문장별 공식 한글 해석)가 있다.
  - **key_grammar.point 는 `key_grammar_hint` 그대로 사용**하라. 이 합본에는 이미 아래 15개 핵심문법이 있으니 **절대 겹치면 안 된다**(배정된 것은 이미 안 겹치게 골라 둠):
    긍정 동의 도치(So+V+S) / would rather A than B / 명사절 접속사 that / 전치사+관계대명사(to which) / A rather than B / 이중부정 / 병렬구조 / 관계대명사 what / 양보 although / 동명사 주어 수일치 / 상관접속사 not A but B / 분사구문 / so+형/부+that / 부정사 부사적 용법(목적) / the 비교급~the 비교급.
  - source_sentence 는 그 문법이 실제로 든 이 소단원 문장 원문.
- **끊어읽기 ko(한글)는 `ko_gloss` 의 표현을 최대한 따르되**, '중간(절~큰 구, 문장당 2~4조각)' 세기로 끊고 각 내용어 조각에 단어 빈칸 1개를 넣어라.
- **misreads(오답)**: 수능형답게 '핵심 논지/함축/지칭/논리 인과'를 겨냥(어휘 오역 금지). 각 소단원에 지칭·함축 최소 1개씩.
- stance/structure: 지문 성격에 맞게(인간 본성·논설 = 주장→근거·예시 또는 예시→일반화(결론); 명왕성 일화 = 시간·순서(나열)). 값은 허용된 것만.
