...
놓는다. 원문에 연결어가 없는 지문이 오히려 많은데, 그때는 앞뒤 관계가 뚜렷한 자리에
빈칸을 새로 놓고 들어갈 연결어를 만들어 낸다(원문에 없어도 문항은 성립한다 —
학생이 판정하는 것은 '원문에 무엇이 있었나'가 아니라 '앞뒤가 어떤 관계인가'다).

선지는 '한 자리만 보고' 풀리지 않게 짠다. 정답의 (A) 낱말이 그 선지에만 있으면
학생은 (B) 를 보지도 않고 답을 고른다. 그래서 정답의 (A)·(B) 낱말이 저마다 오답에도
한 번 더 나오도록 강제한다(shape.check_linker_pairs).
"""
...
- 원문에 연결어가 있는 문장이면 그것을 고르는 편이 좋습니다(원래 글이 그 자리에서
  관계를 밝혔다는 뜻이라 정답이 확실해집니다). **다만 그런 문장이 없어도 됩니다.**
  원문에 연결어가 하나도 없는 지문이 오히려 흔한데, 그때는 앞뒤 관계가 뚜렷한 자리를
  골라 빈칸을 새로 놓고 들어갈 연결어를 만들어 내세요. 학생이 판정하는 것은 '원문에
  무엇이 있었나'가 아니라 '앞뒤 두 문장이 어떤 관계인가'입니다.
  · 관계가 뚜렷한 자리란: 앞 문장을 뒤집는 자리 · 앞을 예시로 받는 자리 · 앞의 결과를
    잇는 자리 · 앞에 하나를 더 얹는 자리 · 앞을 바꿔 말하는 자리.
  · 관계가 흐릿한 자리(그냥 다음 이야기로 넘어가는 자리)는 고르지 마세요 — 어느
    연결어를 넣어도 말이 되어 복수정답이 됩니다.
- 두 빈칸은 서로 '다른 관계'여야 합니다(둘 다 대조면 한 번만 알면 둘 다 풀립니다).

[선지] pairs 5개 — 각 항목은 a((A)에 들어갈 말), b((B)에 들어갈 말).
- answer_no 한 개만 두 자리 모두 맞습니다.
- 나머지 4개는 적어도 한 자리가 확실히 틀려야 합니다.

- **한 자리만 보고 답이 나오면 안 됩니다 — 이 유형에서 가장 중요합니다.**
  정답의 (A) 낱말은 오답에도 **최소 한 번 더** 나와야 하고, 정답의 (B) 낱말도
  마찬가지입니다. 그래야 학생이 (A) 를 맞혀도 후보가 둘 남아 (B) 까지 따지게 됩니다.
  (A) 자리에 쓰는 서로 다른 낱말은 **2~3가지**로 줄이고, (B) 도 그렇게 하세요.
      좋음: ①(A)However-(B)Therefore ②(A)However-(B)Moreover
            ③(A)In addition-(B)Therefore ④(A)In addition-(B)Moreover
            ⑤(A)For example-(B)Instead          ← 정답이 ①이면 (A)도 (B)도 겹친다
      나쁨: 다섯 선지의 (A) 가 모두 다른 낱말 — (A) 하나만 판정하면 끝난다.
- 겹쳐 쓰는 오답 낱말은 '그 자리에서 확실히 틀린' 것으로 고르세요. 정답과 바꿔 써도
  말이 되는 낱말(Therefore 와 Thus, However 와 Nevertheless)은 같은 자리에 함께
  두면 안 됩니다 — 복수정답이 됩니다.
...
def generate(client: ClaudeClient, analysis: Analysis, body: str,
    ...
    def _chk(out: LinkerOut) -> None:
        ...
        # 한 자리만 보고 답이 나오면 두 자리를 낸 뜻이 없다.
        flaw = shape.check_linker_pairs(out.pairs, out.answer_no)
        if flaw:
            raise ValueError("연결어 선지 설계 결함 — " + " ".join(flaw))
