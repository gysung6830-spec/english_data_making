...
[오답 4개] 아래 함정 기법을 '서로 다르게' 하나씩 사용하고, 각 오답에는 지문에 실제로 나온
단어를 일부 그대로 노출해 정답처럼 보이게 하세요.
  (1) 부분 일치 + 한 요소만 왜곡: 문장 대부분은 지문과 맞고, 숫자·범위·조건 '한 군데'만 틀림.
  (2) 주체 바꿔치기: 지문의 진짜 관계에서 행위자/대상만 맞바꿈(단어는 다 지문에 있음).
  (3) 인과 날조·역전: 두 사실은 맞되 지문에 없는 인과(~때문에/~하므로)를 끼워 넣음.
  (4) 정도·범위 과장: 지문의 완곡어(often, may, some, one)를 절대어(only, always, must, 유일한,
      가장 큰)로 바꿔 과장.
  ※ 또는 '미언급인데 그럴듯'(지문과 모순은 아니나 아예 언급되지 않은 상식적 진술)을 섞어도 됨.
...
def generate(client: ClaudeClient, analysis: Analysis, body: str,
             max_retries: int = 1, difficulty: str = HARD) -> tuple[str, str]:
