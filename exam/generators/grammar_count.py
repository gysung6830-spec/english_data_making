...
발문은 '밑줄 친 부분 중, 어법상 틀린 것의 개수는?'이고 선지는 ①1개 ②2개 ③3개 ④4개 ⑤5개
입니다. 학생은 밑줄을 하나하나 따져 '몇 개가 틀렸는지'를 셉니다.

{rules}
[2단계 — 밑줄과 오류 심기] marks / wrong_nos / reasons
- marks: 밑줄 5~7개. 각 항목은 sent_no(1-based, **다시 쓴 지문 기준**), word(다시 쓴 지문에
  실제로 있는 낱말), shown(문제에 보여줄 형태). 읽는 순서대로 나열하세요.
- wrong_nos: 그중 어법상 '틀린' 밑줄 번호들 — 1개 이상 5개 이하. shown 을 틀린 형태로 바꿉니다.
  옳은 밑줄은 shown 을 word 와 똑같이 둡니다.
- 틀린 개수를 매번 같게 하지 마세요(2개나 3개로 고정되면 학생이 세지 않고 찍습니다).
- 타깃 문법: 수 일치 / 시제 / 태 / 준동사 / 관계사 / 병렬 / 대명사 / 비교.

[근거] reasons 는 **밑줄 전부**에 대해 하나씩 씁니다(밑줄이 6개면 6개).
...
- 옳은 밑줄은 '완전히' 옳아야 합니다. 조금이라도 어색하면 개수가 흔들립니다.
