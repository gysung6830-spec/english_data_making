...
def _dna() -> Passage:
    ...
    p.set_qa(CONTENT_2, *B.make_content_ox(
        ...
        reasons=[
            "지문은 DNA를 '자연이 만든(nature's own)' 하드 드라이브라고 했다. 만든 주체가 기술자로 바뀌었다.",
            "첫 문장 'Every living cell carries a molecule called DNA, which works as "
            "nature's own hard drive' 를 그대로 옮긴 일치 진술이다.",
            "지문은 DNA가 오래 견딘다는 것과 연구자들이 파일을 기록한다는 것을 나란히 말했을 뿐, 둘을 인과로 엮은 적이 "
            "없다.",
            "지문은 '그 효율에 영감을 받아(Inspired by such efficiency)' 연구가 시작됐다고 했다. "
            "효율이 원인이고 연구가 결과인데 그 방향이 뒤집혔다.",
            "지문은 'in theory(이론상)'라는 단서를 달아 말했다. 그 단서를 떼면 이미 이루어진 일처럼 읽힌다.",
            "지문은 '언젠가(One day) 저장될 수 있다(may be stored)'는 가능성을 말했다. 이미 그렇게 "
            "되었다는 것은 앞뒤가 다르다.",
            "지문은 'not only ~ but also' 로 '저장한다는 것뿐 아니라' 좁은 공간에 담는다는 점도 주목할 "
            "만하다고 했다. only 로 그 대목을 잘라 냈다.",
            "지문은 '엄청난 양을 아주 좁은 공간에 담는다(packs an enormous amount into "
            "a vanishingly small space)'고 했다. 이를 바꿔 표현한 일치 진술이다.",
            "지문은 처음부터 끝까지 DNA를 '저장 매체'로 다룬다. 생물학의 소재로 다룬다는 것은 글의 논지를 반대로 세운 "
            "것이다.",
            "자기 테이프와의 비용 비교는 지문에 없다. 그럴듯하지만 언급되지 않았다.",
        ],
        axes=["주체·대상 바꿔치기", "일치", "인과 날조", "인과 역전", "조건 삭제",
              "시점 뒤집기", "부정 뒤집기", "일치", "논지·화자 뒤집기", "미언급인데 그럴듯"],
    ))
...
def _star() -> Passage:
    ...
    p.set_qa(CONTENT_2, *B.make_content_ox(
        ...
        truths=[False, True, False, False, False,
                False, False, True, False, False],
        reasons=[
            "개인적 탁월함(personal brilliance)을 보상하는 쪽은 실무 역할이고, 관리자 역할은 남을 키우는 "
            "인내를 보상한다. 두 역할의 짝이 뒤바뀌었다.",
            "두 번째 문장 'They assume that a star will naturally make a star boss' 를 "
            "바꿔 표현한 일치 진술이다.",
            "지문에서 승진이 먼저이고 인재를 잃는 것은 그 결과다. 원인과 결과가 뒤집혔다.",
            "지문은 'Placed in charge(자리에 앉혀지자)'라는 조건 아래 그렇게 된다고 했다. 그 조건을 떼면 "
            "사람 자체가 원래 그렇다는 뜻이 된다.",
            "지문은 팀이 정체되고 '그러고 나서(and)' 채용이 실망으로 바뀐다고 했다. 앞뒤 순서가 뒤집혔다.",
            "지문은 두 자리가 '거의 정반대 능력(almost opposite skills)'을 요구한다고 못 박았다. 그 "
            "대목을 아니라고 뒤집었다.",
            "그것은 회사들이 그렇게 '여긴다(They assume)'고 소개한 통념이고, 필자는 바로 다음 문장에서 "
            "however 로 반박한다. 통념을 필자의 주장으로 바꿔치기했다.",
            "다섯 번째 문장 'keeps chasing personal wins and neglects the slow work of "
            "coaching' 을 그대로 옮긴 일치 진술이다.",
            "지문은 스타가 개인 성과를 좇느라 코칭을 소홀히 한다고 했지, '일부러 무시한다'고 하지 않았다. 없던 의도를 "
            "원인으로 지어냈다.",
            "승진 전 관리자 교육은 그럴듯한 해법이지만 지문에 전혀 언급되지 않았다.",
        ],
        axes=["주체·대상 바꿔치기", "일치", "인과 역전", "조건 삭제", "시점 뒤집기",
              "부정 뒤집기", "논지·화자 뒤집기", "일치", "인과 날조", "미언급인데 그럴듯"],
    ))
