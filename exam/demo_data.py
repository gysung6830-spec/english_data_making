...
def _passage_dna() -> Passage:
    ...
    p.set_qa(CONTENT, *B.make_content_ox(
        ...
        statements=[
            ...
            "합성 DNA에 기록된 정보는 어떤 조건에서든 반드시 수천 년간 보존된다.",
            "과학자들은 이미 인류의 모든 도서관 자료를 DNA에 저장해 두었다.",
            "DNA에 정보를 새기는 기술은 아직 속도가 느리고 비용이 많이 든다.",
            "DNA는 정보를 저장하는 능력만으로 주목받는 것이 아니라, 그것을 극히 좁은 "
            "공간에 담아내는 점에서도 주목받는다.",
            "일반 하드 드라이브 수백만 개가 DNA 1그램만큼의 정보를 담을 수 있다.",
            "DNA는 언제나 예외 없이 유일한 장기 보관 수단으로 쓰인다.",
        ],
        truths=[False, False, True, False, False,
                False, True, False, False, False],
        reasons=[
            "부호화하는 대상은 '합성 DNA(synthetic DNA)'이고, 일반 하드 드라이브는 용량을 "
            "비교하는 대상으로만 나온다. 대상이 뒤바뀌었다. (주체·대상 바꿔치기)",
            "DNA가 안정적이라 뼈·얼음 속에서 오래 보존된다는 것은 맞지만, 지문은 '수만 년"
            "(tens of thousands of years)'이라 했다. 기간 한 요소만 다르다. "
            "(부분 일치 + 한 요소 왜곡)",
            "지문은 '1그램의 DNA가 이론상 일반 하드 드라이브 수백만 개와 맞먹는 데이터를 "
            "담을 수 있다'고 했다. 원문 단어를 숨기고 '아주 적은 양·막대한 데이터'로 "
            "바꿔 표현한 일치 진술이다.",
            "지문은 밀도(작은 공간에 많이 담음)와 내구성(오래 견딤)을 나란히 든 장점일 뿐, "
            "'오래 보존하기 때문에 작아졌다'는 인과를 말한 적이 없다. (인과 날조)",
            "지문은 '수천 년간 보존될 수 있게 한다(allows ~ to be preserved)'고 했을 뿐 "
            "'어떤 조건에서든 반드시'라고는 하지 않았다. (정도·빈도 과장)",
            "지문은 '언젠가(one day) 저장될 수 있다(may be stored)'는 가능성을 말했을 뿐, "
            "'이미 저장해 두었다'는 내용은 없다. (시점 뒤집기)",
            "지문은 'The technique is still slow and expensive' 라고 밝혔다. 그대로 "
            "옮긴 일치 진술이다.",
            "지문은 정확히 그 반대로 말한다 — 저장한다는 점뿐 아니라 '엄청난 양을 아주 "
            "좁은 공간에 담는다'는 점이 주목할 만하다고 했다. 앞뒤 두 요소는 맞지만 어느 "
            "쪽이 주목받는 점인지가 뒤집혔다. (인과·관계 역전)",
            "지문은 DNA 1그램이 하드 드라이브 수백만 개 '만큼' 담는다고 했다. 방향이 "
            "뒤집혀 담는 쪽과 담기는 쪽이 맞바뀌었다. (주체·대상 바꿔치기)",
            "장기 보관 수단이 DNA뿐이라는 말은 지문에 없다. 그럴듯하지만 언급되지 "
            "않았다. (미언급인데 그럴듯)",
        ],
    ))
...
def _passage_star() -> Passage:
    ...
    p.set_qa(CONTENT, *B.make_content_ox(
        ...
        truths=[False, False, True, False, False,
                False, True, False, False, False],
        reasons=[
            "개인적 탁월함(personal brilliance)을 보상하는 것은 '실무 역할'이고 관리자 "
            "역할은 '남을 키우는 인내'를 보상한다. 두 역할의 짝이 정확히 뒤바뀌었다. "
            "(주체 바꿔치기)",
            "앞부분(뛰어난 실무자를 잃음)은 맞지만, 지문은 회사가 유능한 관리자도 "
            "'잃는다(loses both ~ and a capable manager)'고 했다. 한 요소만 뒤집었다. "
            "(부분 일치 + 한 요소 왜곡)",
            "첫 문장 'Many companies promote their very best performer into a "
            "management role' 을 그대로 옮긴 일치 진술이다.",
            "지문은 팀 정체의 이유를 '스타가 개인 성과를 좇고 코칭을 소홀히 하기 "
            "때문'이라 했지, '코칭 기술을 배운 적이 없어서'라는 구체 원인은 대지 "
            "않았다. (인과 날조)",
            "지문은 그런 일이 흔히 일어난다고 말할 뿐 '언제나 예외 없이'라고 하지 "
            "않았다. (정도·빈도 과장)",
            "승진 전 관리자 교육은 그럴듯한 해법이지만 지문에 전혀 언급되지 않았다. "
            "(미언급인데 그럴듯)",
            "세 번째 문장 'The two jobs, however, demand almost opposite skills' 를 "
            "바꿔 표현한 일치 진술이다.",
            "지문에서 스타가 코칭을 소홀히 하는 것은 '두 자리가 정반대 능력을 요구하기 "
            "때문'이지 승진 자체가 원인이라고 하지 않았다. 원인과 결과가 뒤집혔다. "
            "(인과 역전)",
            "지문은 회사가 지금도 그렇게 '가정한다(They assume)'고 했다. 이미 버렸다는 "
            "것은 정반대다. (시점 뒤집기)",
