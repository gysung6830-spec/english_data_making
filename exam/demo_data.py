...
def _passage_dna() -> Passage:
    ...
    p.set_qa(CONTENT, *B.make_content(
        ...
        choices=[
            # ① 정도·범위 과장
            "DNA는 정보를 매우 촘촘하게 저장하며, 그 밀도는 어떤 저장 장치도 결코 따라올 수 없다.",
        ...
        wrong={
            1: "지문은 DNA가 '아주 작은 공간(vanishingly small space)'에 담는다고 했을 뿐, '어떤 "
               "장치도 결코 못 따라온다'는 절대적 우열은 말하지 않았다. '결코 ~없다'가 과장이다. "
               "(정도·범위 과장)",
            3: "부호화하는 대상은 '합성 DNA(synthetic DNA)'이고, 일반 하드 드라이브는 용량을 "
               "비교하는 대상으로만 나온다. 대상이 뒤바뀌었다. (대상 바꿔치기)",
            4: "지문은 기술이 '느리고 비싸다'는 것과 '수천 년 보존된다'는 것을 but 으로 나란히 "
               "제시할 뿐, 둘 사이의 인과(~때문에)는 없다. (인과 날조)",
            5: "지문은 '언젠가(one day) 저장될 수 있다(may be stored)'는 미래 가능성을 말했는데, "
               "선지는 '이미 저장해 두었다'며 완료된 사실로 왜곡했고 그런 내용은 지문에 없다. "
               "(미언급·시제 왜곡)",
        },
    ))
...
def _passage_star() -> Passage:
    ...
    p.set_qa(CONTENT, *B.make_content(
        ...
        choices=[
            ...
            # ③ 정도·범위 과장(often → always)
            "최고 성과자를 관리자로 올리는 것은 항상 회사에 손해를 끼친다.",
            # ④ 인과 날조(구체 원인 지어냄)
        ...
        wrong={
            ...
            3: "지문은 그런 승진이 '대개(often)' 문제가 된다고 했을 뿐 '항상(always)' 손해라고 "
               "단정하지 않았다. '항상'이라는 절대어가 과장이다. (정도·범위 과장)",
            4: "지문은 팀 정체의 이유를 '스타가 개인 성과를 좇고 코칭을 소홀히 하기 때문'이라 "
               "했지, '코칭 기술을 배운 적이 없어서'라는 구체 원인은 대지 않았다. (인과 날조)",
