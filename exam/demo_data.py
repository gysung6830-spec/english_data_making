...
def _passage_dna() -> Passage:
    ...
    # ⑥ 내용 O/X — 다섯 진술을 각각 판정. O 는 둘, X 는 셋.
    p.set_qa(CONTENT, *B.make_content_ox(
        ...
        reasons=[
            "지문은 '1그램의 DNA가 이론상 일반 하드 드라이브 수백만 개와 맞먹는 데이터를 담을 "
            "수 있다(A single gram of DNA ~ millions of ordinary hard drives)'고 했다. 원문 "
            "단어를 숨기고 '아주 적은 양·막대한 데이터'로 바꿔 표현한 일치 진술이다.",
            "DNA가 안정적이라 뼈·얼음 속에서 오래 보존된다는 것은 맞지만, 지문은 '수만 년"
            "(tens of thousands of years)'이라 했다. '수백 년'이라는 기간 한 요소만 다르다. "
            "(부분 일치 + 한 요소 왜곡)",
            "부호화하는 대상은 '합성 DNA(synthetic DNA)'이고, 일반 하드 드라이브는 용량을 "
            "비교하는 대상으로만 나온다. 대상이 뒤바뀌었다. (주체·대상 바꿔치기)",
            "지문은 'The technique is still slow and expensive' 라고 밝혔다. 그대로 옮긴 "
            "진술이다.",
...
def _passage_star() -> Passage:
    ...
    p.set_qa(CONTENT, *B.make_content_ox(
        ...
        truths=[False, True, False, True, False],
        reasons=[
            "개인적 탁월함(personal brilliance)을 보상하는 것은 '실무 역할'이고 관리자 역할은 "
            "'남을 키우는 인내'를 보상한다. 두 역할의 짝이 정확히 뒤바뀌었다. (주체 바꿔치기)",
            "지문은 '자리에 앉은 former star가 개인적 성과를 계속 좇고 팀 코칭의 더딘 일을 "
            "소홀히 한다(keeps chasing personal wins and neglects ~ coaching)'고 했다. 이를 "
            "유의어로 바꿔 표현한 일치 진술이다.",
            "앞부분(뛰어난 실무자를 잃음)은 맞지만, 지문은 회사가 유능한 관리자도 '잃는다"
            "(loses both ~ and a capable manager)'고 했다. '관리자는 얻는다'로 한 요소만 "
            "뒤집었다. (부분 일치 + 한 요소 왜곡)",
            "첫 문장 'Many companies promote their very best performer into a management "
            "role' 을 그대로 옮긴 진술이다.",
            "승진 전 관리자 교육은 그럴듯한 해법이지만 지문에 전혀 언급되지 않았다. (미언급)",
        ],
    ))
