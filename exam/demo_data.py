...
def _passage_dna() -> Passage:
    ...
    # ⑥ 내용 O/X — 열 진술 각각 판정(O 는 늘 둘). 한글판·영어판이 서로 다른 사실을 묻는다.
    p.set_qa(CONTENT, *B.make_content_ox(
        s,
        statements=[
            "연구자들은 디지털 파일을 일반 하드 드라이브에 부호화하기 시작했다.",
            "DNA가 정보를 담는 능력은 뛰어나지만, 그 양이 특별히 많은 것은 아니다.",
            "아주 적은 양의 DNA만으로도 이론상 막대한 양의 데이터를 저장할 수 있다.",
            "DNA가 정보를 오래 보존하기 때문에 세포는 아주 작은 공간에 정보를 담게 되었다.",
            "1그램의 DNA가 하드 드라이브 수백만 개 분량을 담는 것은 이미 실현된 성과다.",
            "과학자들은 이미 인류의 모든 도서관 자료를 DNA에 저장해 두었다.",
            "DNA에 정보를 새기는 기술은 아직 속도가 느리고 비용이 많이 든다.",
            "연구자들이 합성 DNA에 파일을 기록하기 시작한 덕분에 DNA가 정보를 촘촘히 "
            "담을 수 있게 되었다.",
            "필자는 DNA 저장 기술이 앞으로도 실용화되기 어렵다고 본다.",
            "DNA는 언제나 예외 없이 유일한 장기 보관 수단으로 쓰인다.",
        ],
        truths=[False, False, True, False, False,
                False, True, False, False, False],
        reasons=[
            "부호화하는 대상은 '합성 DNA(synthetic DNA)'이고, 일반 하드 드라이브는 용량을 비교하는 대상으로만 "
            "나온다. 대상이 뒤바뀌었다.",
            "지문은 정확히 그것을 못 박아 부정한다 — 주목할 만한 점은 저장한다는 것뿐 아니라 '엄청난 양(an "
            "enormous amount)'을 담는다는 것이라고 했다. 지문이 강조한 대목을 아니라고 뒤집었다.",
            "지문은 '1그램의 DNA가 이론상 일반 하드 드라이브 수백만 개와 맞먹는 데이터를 "
            "담을 수 있다'고 했다. 원문 단어를 숨기고 '아주 적은 양·막대한 데이터'로 "
            "바꿔 표현한 일치 진술이다.",
            "지문은 밀도(작은 공간에 많이 담음)와 내구성(오래 견딤)을 나란히 든 장점일 뿐, '오래 보존하기 때문에 "
            "작아졌다'는 인과를 말한 적이 없다.",
            "지문은 'in theory(이론상)'라는 단서를 달아 말했다. 그 단서를 떼면 이미 이루어진 일처럼 읽혀 지문이 "
            "말한 것과 달라진다.",
            "지문은 '언젠가(one day) 저장될 수 있다(may be stored)'는 가능성을 말했을 뿐, '이미 저장해 "
            "두었다'는 내용은 없다.",
            "지문은 'The technique is still slow and expensive' 라고 밝혔다. 그대로 "
            "옮긴 일치 진술이다.",
            "지문은 '그 효율에 영감을 받아(Inspired by such efficiency)' 연구가 시작됐다고 했다. "
            "촘촘히 담는 성질이 원인이고 연구가 결과인데, 그 방향이 뒤집혔다.",
            "기술이 아직 느리고 비싸다는 것은 맞지만, 필자는 마지막 문장에서 '언젠가 우리의 도서관과 사진이 분자 안에 "
            "안전하게 담길지도 모른다'며 낙관으로 글을 맺는다. 사실은 맞아도 필자의 결론은 정반대다.",
            "장기 보관 수단이 DNA뿐이라는 말은 지문에 없다. 그럴듯하지만 언급되지 않았다.",
        ],
        axes=["주체·대상 바꿔치기", "부정 뒤집기", "일치", "인과 날조", "조건 삭제",
              "시점 뒤집기", "일치", "인과 역전", "논지·화자 뒤집기", "미언급인데 그럴듯"],
    ))
...
def _passage_star() -> Passage:
    ...
    p.set_qa(CONTENT, *B.make_content_ox(
        ...
        statements=[
            ...
            "필자는 뛰어난 성과자가 자연히 뛰어난 상사가 된다고 본다.",
            "많은 회사가 가장 뛰어난 성과자를 관리자 자리로 승진시킨다.",
            "팀이 정체되는 것은 새 관리자가 코칭 기술을 배운 적이 없기 때문이다.",
            "관리자 자리가 요구하는 능력은 실무 자리가 요구하는 능력과 크게 다르지 않다.",
            "회사는 이런 문제를 막기 위해 승진 전에 관리자 교육을 실시해야 한다.",
            "두 자리는 요구하는 능력이 거의 정반대다.",
            "승진한 스타가 팀을 소홀히 하는 것은 회사가 그를 승진시킨 결과다.",
            "회사는 스타가 스타 상사가 될 것이라는 기대를 이미 버렸다.",
            "팀이 정체된 뒤에야 그 채용이 실망으로 바뀐다.",
        ],
        truths=[False, False, True, False, False,
                False, True, False, False, False],
        reasons=[
            "개인적 탁월함(personal brilliance)을 보상하는 것은 '실무 역할'이고 관리자 역할은 '남을 키우는 "
            "인내'를 보상한다. 두 역할의 짝이 정확히 뒤바뀌었다.",
            "그것은 회사들이 그렇게 '여긴다(They assume)'고 소개한 통념이고, 필자는 바로 다음 문장에서 "
            "however 로 그것을 반박한다. 통념을 필자의 주장으로 바꿔치기했다.",
            "첫 문장 'Many companies promote their very best performer into a "
            "management role' 을 그대로 옮긴 일치 진술이다.",
            "지문은 팀 정체의 이유를 '스타가 개인 성과를 좇고 코칭을 소홀히 하기 때문'이라 했지, '코칭 기술을 배운 적이 "
            "없어서'라는 구체 원인은 대지 않았다.",
            "지문은 두 자리가 '거의 정반대 능력(almost opposite skills)'을 요구한다고 못 박았다. 그 "
            "대목을 아니라고 뒤집었다.",
            "승진 전 관리자 교육은 그럴듯한 해법이지만 지문에 전혀 언급되지 않았다.",
            "세 번째 문장 'The two jobs, however, demand almost opposite skills' 를 "
            "바꿔 표현한 일치 진술이다.",
            "지문에서 스타가 코칭을 소홀히 하는 것은 '두 자리가 정반대 능력을 요구하기 때문'이지 승진 자체가 원인이라고 "
            "하지 않았다. 원인과 결과가 뒤집혔다.",
            "지문은 회사가 지금도 그렇게 '가정한다(They assume)'고 했다. 이미 버렸다는 것은 정반대다.",
            "지문은 팀이 정체되는 것과 채용이 실망으로 바뀌는 것을 and 로 나란히 "
            "놓았을 뿐, '정체된 뒤에야'라는 선후를 정하지 않았다. (조건·순서 왜곡)",
        ],
        axes=["주체·대상 바꿔치기", "논지·화자 뒤집기", "일치", "인과 날조", "부정 뒤집기",
              "미언급인데 그럴듯", "일치", "인과 역전", "시점 뒤집기", "일치"],
    ))
