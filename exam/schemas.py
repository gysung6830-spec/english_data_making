...
N_OX_SHORT = 8     # 재료가 모자란 지문의 영어판 — 아래 주석 참고
N_OX_TRUE = 2      # 그중 O(일치)인 것 — 늘 2개, 나머지는 모두 X

# 재료를 세는 자는 '문장 수'가 아니라 '낱말 수'다.
# 실제 EBS 지문 둘을 재어 보고 고쳤다.
#     지문1  5문장 141낱말 → 판정 가능한 명제 약 13개 (문장이 길고 종속절이 많다)
#     지문2 10문장 132낱말 → 약 13~16개
# 문장 수는 두 배 차이인데 명제 수는 비슷하다. 낱말 수가 거의 같으니 당연하다.
# 문장 수로 자르면 정보가 더 많은 지문1이 오히려 축소 대상이 된다 — 축이 틀렸다.
#
# 줄일 때 영어판을 줄이는 까닭: 영어판은 '한글판이 쓰지 않은 사실'을 받아 만들므로
# 모자라는 쪽은 늘 영어판이다(10+10 → 10+8).
_OX_SIZES = (N_OX_SHORT, N_OX)

# 이 아래로는 O 넷을 서로 다른 명제에서 뽑기 어렵다(명제 ≈ 낱말/10 이므로 명제 10개 선).
OX_WORDS_SHORT = 100
# 이 아래로는 여덟 축의 재료(인과·조건·시점·통념)가 다 있지 않다.
OX_WORDS_THIN = 120


def passage_words(sentences: list[str]) -> int:
    return sum(len(s.split()) for s in sentences)


def ox_sizes(sentences: list[str]) -> tuple[int, int]:
    """이 지문에서 만들 진술 수 (한글판, 영어판).

    낱말이 OX_WORDS_SHORT 미만이면 서로 다른 사실이 모자라 억지 진술이 나온다.
    그때만 영어판을 여덟으로 줄인다(총 18개). 그 위로는 늘 열 개씩이다.
    """
    return (N_OX,
            N_OX_SHORT if passage_words(sentences) < OX_WORDS_SHORT else N_OX)
