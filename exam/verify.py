...
HIGH_RISK: dict[str, str] = {
    ...
    # ---- 어휘 유의어형 ----
    # 어휘 3종 중 이것만 검증한다. 나머지 둘(원문단어형·부정어형)은 정답이 아닌 밑줄을
    # 원문 낱말 그대로 두는 설계라, 코드가 그 상태를 확정적으로 복원한다
    # (generators/vocab._restore_original_marks). 원문 낱말은 필자가 쓴 말이므로
    # 어색할 수 없고, 그래서 정답이 둘이 될 수 없다 — 물어볼 것이 남지 않는다.
    # 유의어형만은 오답 넷을 '일부러' 바꿔 놓는 방식이라 그 넷이 문맥에 맞는지를
    # 판단해야 하고, 판단은 코드가 못 한다.
    #
    # 키를 'vocab' 이 아니라 'vocab_synonym' 으로 둔 까닭: _base_key 가 슬롯키의 끝자리
    # 숫자만 떼므로 'vocab' 이라 적으면 vocab_2·vocab_3 까지 고위험으로 딸려 들어와
    # 지문당 검증이 1회가 아니라 3회가 된다. 이 키는 어느 슬롯키로도 정규화되지 않아
    # generators/vocab 이 명시적으로 부를 때만 쓰인다.
    "vocab_synonym": "정답이 아닌 밑줄 넷이 바뀐 뒤에도 문맥상 '완전히' 자연스러운가. "
                     "하나라도 어색하면 정답이 둘이 되어 결함이다. 그리고 정답 밑줄은 "
                     "'확실히' 문맥에 어긋나는가(반의어로 바뀌었는지). 판단이 갈릴 "
                     "여지가 있으면 결함이다.",
}
