...
# 우리가 기본값으로 넣었다가 잘못이었던 문구. 배포해도 안 덮이는 디스크에
# 이미 심겨 있어, 파일을 고치는 것만으로는 안 내려갑니다. 토씨 하나까지 같을
# 때만 바꿉니다 — 사장님이 손수 쓰신 글은 건드리면 안 됩니다.
RETIRED_COPY = {
    # 보내 주신 그 시험지를 고쳐서 돌려주는 것처럼 읽혔습니다.
    # 실제로 돌아가는 것은 할인 쿠폰입니다.
    "학교 시험지를 보내 주시면, 더 정확한 자료로 돌려드립니다":
        "학교 시험지를 보내 주시면, 할인 쿠폰으로 돌려드립니다",
    "학교 시험지를 나눠 주시면, 더 정확한 자료로 돌려드립니다":
        "학교 시험지를 보내 주시면, 할인 쿠폰으로 돌려드립니다",
    "학교 시험지를 보내 주시면 | 더 정확한 자료로 돌려드립니다":
        "학교 시험지를 보내 주시면 | 할인 쿠폰으로 돌려드립니다",
    "학교 시험지를 나눠 주시면 | 더 정확한 자료로 돌려드립니다":
...
WORD_GAMES = {
    # 짝 맞추기(카드 뒤집기)는 뺐습니다. 카드가 어디 있었는지만 외우면
    # 단어를 몰라도 이겨서, 게임은 되는데 공부가 안 됩니다.
    "rain": {"name": "단어 비", "mark": "🌧",
             "lead": "떨어지는 단어에서 그 뜻을 찾아 터뜨리세요.",
             "how": "아래 뜻에 맞는 단어를 누르면 터집니다. 바닥에 닿으면 목숨이 하나 줄고, "
                    "갈수록 빨라집니다.",
             "best": 200},
...
# 패키지 상자에 걸 한 줄. 자료 이름만 늘어놓으면 '좌지문우해석' 이 무엇인지
# 모르는 손님은 그냥 지나칩니다. 이름들이 합쳐서 무슨 순서가 되는지 풀어 줍니다.
# 문항 수를 세는 패키지(문제)는 화면에서 수를 더해 쓰므로 여기 없습니다.
# 디스크에 있는 옛 자료 파일에는 이 문구가 없으니 기본값을 코드에 둡니다.
# 핵심어와 풀이를 따로 둡니다. 문제 패키지가 문항 수를 굵게 쓰는 그 자리에
# 이 묶음의 핵심어가 놓이도록, 화면에서 앞쪽만 굵게 씁니다. 한 덩어리 글에
# 태그를 섞어 두면 나중에 자료 파일에서 고칠 때 태그까지 지켜야 합니다.
PKG_COVERS = {
    "analysis": ("능동적 분석 독해",
                 "읽고 · 끊고 · 뜯고 · 쓰고 · 외우고, 지문 하나를 그 순서로 끝냅니다"),
}
...
# 빈칸 난이도 — 가리는 양만 늘리면 3단계쯤에서 갑자기 벽이 섭니다. 그래서
# 두 축으로 올립니다. 먼저 **힌트**를 하나씩 거두고(첫 글자 → 글자 수 → 없음),
# 그다음 **범위**를 넓힙니다(내용어 → 기능어까지). 어느 한 단계도 앞 단계보다
# 두 가지가 한꺼번에 어려워지지 않게 짰습니다.
#
#   ratio  가릴 비율        scope  content=내용어만 · all=기능어까지
#   hint   first=첫 글자 남김 · len=칸 길이가 글자 수 · none=아무것도 안 알려 줌
BLANK_LEVELS = [
    {"no": 1, "name": "첫 글자", "ratio": 0.35, "scope": "content", "hint": "first",
     "desc": "내용어를 가리되 첫 글자를 남깁니다"},
    {"no": 2, "name": "글자 수", "ratio": 0.35, "scope": "content", "hint": "len",
     "desc": "같은 자리를 첫 글자 없이. 칸 길이가 글자 수입니다"},
    {"no": 3, "name": "내용어 전부", "ratio": 1.00, "scope": "content", "hint": "len",
     "desc": "내용어를 남김없이. 칸 길이는 아직 알려 줍니다"},
    {"no": 4, "name": "힌트 없이", "ratio": 1.00, "scope": "content", "hint": "none",
     "desc": "같은 자리를 칸 길이도 없이"},
    {"no": 5, "name": "기능어까지", "ratio": 0.70, "scope": "all", "hint": "none",
     "desc": "the·of 같은 자리까지 가립니다"},
    {"no": 6, "name": "통암기", "ratio": 1.00, "scope": "all", "hint": "none",
     "desc": "전부 가립니다"},
]
